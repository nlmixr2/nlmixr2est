...
buildInstaller <- function(name = "nlmixr") {
  rtools <- gsub("/", "\\", utils::getFromNamespace(".rxRtoolsBaseWin", "rxode2")(), fixed = TRUE)
  python <- gsub("/", "\\", utils::getFromNamespace(".rxPythonBaseWin", "rxode2")(), fixed = TRUE)
  ...
  full.ver <- gsub(
    "Rtools version ",
    "",
    readLines(file.path(utils::getFromNamespace(".rxRtoolsBaseWin", "rxode2")(), "VERSION.txt"))
  )
