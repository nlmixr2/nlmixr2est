...
test_install <- function() {
  ...
  if (Sys.info()['sysname'] %in% c('Darwin', 'Linux')) {
    ...
    {
      sink("/dev/null")
      fit1 <- suppressWarnings(suppressMessages(nlmixr(testmod, theo_sd, est = "nlme")))
      sink()
    }
    message("Running saem")
    {
      sink("/dev/null")
      fit2 <- suppressWarnings(suppressMessages(nlmixr(testmod, theo_sd, est = "saem")))
      sink()
    }
    message("Running focei")
    {
      sink("/dev/null")
      fit3 <- suppressWarnings(suppressMessages(nlmixr(testmod, theo_sd, est = "focei")))
      sink()
    }
  } else {
    message("Running nlme")
