...
nmTest({
  ...
  test_that("est='npag' returns a valid nlmixr2 fit with support-point outputs", {
    ...
    expect_equal(nrow(f$env$npagPosteriorEta), length(unique(nlmixr2data::theo_sd$ID)))
  ...
  test_that("est='npag' residual optimization moves the residual and certifies D(F)", {
    ...
    f0 <- nlmixr2(
      ...
      nlmixr2data::theo_sd,
      est = "npag",
      control = npagControl(points = 256L, cycles = 15L, gammaOptimize = FALSE, residOptimize = "none")
    )
    ...
    expect_false(isTRUE(all.equal(as.numeric(fg$theta[["add.sd"]]), as.numeric(f0$theta[["add.sd"]])))) # residual moved
