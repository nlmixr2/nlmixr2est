test_that("built-in nlminb automatically uses supported outer curvature", {
  ...
  data <- data.frame(
    ID = rep(1:6, each = 3),
    TIME = rep(1:3, 6),
    DV = c(1.5, 1.7, 1.6, 0.8, 1, 1.1, 1.1, 1.2, 1, 2, 2.1, 1.9, 0.9, 0.8, 0.7, 1.5, 1.3, 1.4),
    AMT = 0,
    EVID = 0
  )
  ...
  for (family in c("FOCEI", "FOCE+")) {
    for (inner in c("n1qn1", "trust")) {
      ...
      control$outerOpt <- -1L
      control$outerOptFun <- gradientOptimizer
