...
nmTest({
  ...
  test_that("the near-tie note fits the runInfo one-line budget", {
    expect_identical(.vaeColinearMsg(0L), character(0))
    expect_identical(.vaeColinearMsg(NA_integer_), character(0))
    msg <- .vaeColinearMsg(2L)
  ...
  test_that("the diagonal-omega advisory fits the budget and gates correctly", {
    ...
    expect_identical(.vaePhiDiagMsg(0L, FALSE, TRUE), character(0))
    expect_identical(.vaePhiDiagMsg(3L, TRUE, TRUE), character(0))
    expect_identical(.vaePhiDiagMsg(3L, FALSE, FALSE), character(0))
    msg <- .vaePhiDiagMsg(3L, FALSE, TRUE)
    expect_gt(length(msg), 0L)
    expect_true(all(nchar(msg) <= 75L))
    expect_false(any(grepl("^vae", msg)))
  })
  ...
  test_that("alternates pair each mate with the covariate it stands in for", {
    ...
    got <- .vaeColinearAlt(list(altTie = tie, altGap = gap),
  ...
  test_that("an empty alternates table is a zero-row frame, never NULL", {
    e <- .vaeColinearAlt(NULL, matrix(FALSE, 1L, 1L), 1L, "eta", "WT")
    ...
    ## producing an NA row
    tie <- matrix(1L, 1L, 1L)
    z <- .vaeColinearAlt(list(altTie = tie, altGap = matrix(0, 1L, 1L)),
  ...
  test_that("vaeControl exposes the colinearity knobs with measured defaults", {
    ...
    expect_equal(vaeControl()$covSelectAltTol, 0.75)
