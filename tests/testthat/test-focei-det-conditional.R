test_that("detHessian='conditional' validates its control", {
  ...
  expect_error(foceiControl(fast = TRUE, interaction = FALSE, detHessian = "conditional"), "requires fast")
...
test_that("the full conditional determinant matches its closed form and its gradient", {
  ...
  objAt <- function(level) {
    .nlmixr(rxode2::ini(fitFull$finalUi, level = level), data, "focei", control = .ctl("conditional"))$objf
  }
  h <- 1e-4
