nmTest({
  ...
  test_that("foceiControl(priorMethod=) defaults to auto-detection", {
    ...
    .fitExplicit <- suppressWarnings(suppressMessages(
      nlmixr2(
        ...
        nlmixr2data::theo_sd,
        est = "focei",
        control = foceiControl(priorMethod = "nwpri", maxOuterIterations = 0L, print = 0L)
      )
    ))
    ...
    expect_error(
      suppressWarnings(suppressMessages(
        nlmixr2(
          ...
          nlmixr2data::theo_sd,
          est = "focei",
          control = foceiControl(priorMethod = "tnpri", maxOuterIterations = 0L, print = 0L)
        )
      )),
      "TNPRI"
    )
  ...
  test_that("a theta prior does not disable fast=TRUE's analytic gradient (#931)", {
    skip_on_cran()
    .fit <- suppressWarnings(suppressMessages(
      nlmixr2(.odeThetaPrior, nlmixr2data::theo_sd, est = "foceif", control = foceiControl(print = 0L))
    ))
  ...
  test_that("a prior downgrades covType='analytic' to a finite-difference covariance", {
    ...
    .fit <- suppressWarnings(suppressMessages(
      nlmixr2(
        ...
        nlmixr2data::theo_sd,
        est = "focei",
        control = foceiControl(covMethod = "analytic", maxOuterIterations = 0L, print = 0L)
      )
    ))
  ...
  test_that("a strong theta prior pulls the estimate toward the prior mean (#931)", {
    skip_on_cran()
    .fit <- suppressWarnings(suppressMessages(
      nlmixr2(.oneCmtPrior, nlmixr2data::theo_sd, est = "focei", control = foceiControl(print = 0L))
    ))
  ...
  test_that("a strong omega prior pulls the estimate toward the prior variance (#931)", {
    skip_on_cran()
    .fit <- suppressWarnings(suppressMessages(
      nlmixr2(.odeOmegaPrior, nlmixr2data::theo_sd, est = "foceif", control = foceiControl(print = 0L))
    ))
  ...
  test_that("the omega-prior gradient formula matches central differences (#931)", {
    ...
    for (.priorSpec in c("prior(eta.cl) ~ dnorm(0, 0.3)", "prior(eta.cl) ~ invWishart(4)")) {
      ...
      expect_equal(myGrad, fdGrad, tolerance = 1e-4, label = paste0("Abar formula (", .method, ")"))
    }
  })
  ...
  test_that("a strong prior on an omega COVARIANCE (off-diagonal) element pulls the estimate toward it", {
    ...
    .fit <- suppressWarnings(suppressMessages(
      nlmixr2(m, nlmixr2data::theo_sd, est = "focei", control = foceiControl(print = 0L))
    ))
    expect_true(inherits(.fit, "nlmixr2FitData"))
