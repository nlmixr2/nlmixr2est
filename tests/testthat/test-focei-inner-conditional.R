test_that("conditional curvature preserves the FOCEI marginal objective", {
  ...
  data <- data.frame(
    ID = rep(1:3, each = 3),
    TIME = rep(1:3, 3),
    DV = c(1.5, 1.7, 1.6, 0.8, 1, 1.1, 2, 2.1, 1.9),
    AMT = 0,
    EVID = 0
  )
  for (censoring in c("none", "m2", "m3", "m4")) {
    for (inner in c("trust", "n1qn1")) {
      fitData <- data
      ...
      reference <- vapply(
        ...
        function(rows) {
          objective <- function(eta) {
            ...
            if (censoring %in% c("m2", "m4")) {
              ll <- ll - pnorm(0, prediction, sd, lower.tail = FALSE, log.p = TRUE)
            }
            if (censoring %in% c("m3", "m4")) {
              probability <- pnorm(rows$DV, prediction, sd)
              if (censoring == "m4") {
                probability <- (probability - pnorm(0, prediction, sd)) / pnorm(0, prediction, sd, lower.tail = FALSE)
              }
...
test_that("conditional curvature requires a compatible optimizer", {
  ...
  expect_error(
    foceiControl(fast = TRUE, innerOpt = "n1qn1", innerHessian = "conditional", warm = "save"),
    "requires warm"
  )
  expect_error(
    foceiControl(fast = TRUE, innerOpt = "n1qn1", innerHessian = "conditional", warm = "none"),
    "requires warm"
  )
  # trust uses the curvature at every trial, so warm is irrelevant to it
  expect_no_error(foceiControl(fast = TRUE, innerOpt = "trust", innerHessian = "conditional", warm = "save"))
  expect_no_error(foceiControl(fast = TRUE, innerOpt = "n1qn1", innerHessian = "conditional", warm = "calc"))
})

test_that("conditional curvature is refused with a likelihood contribution", {
  skip_on_cran()
  model <- function() {
    ini({ level <- 0.2; error <- fix(0.2); etaLevel ~ 0.2 })
    model({ prediction <- exp(level+etaLevel); prediction ~ prop(error) })
  ...
  # register first, arm removal immediately: the fit below is expected to error
  # and must not leave the hook registered for the rest of the suite
  ...
  # the conditional assembly replaces likInner0 and would drop the contribution.
  # The refusal is unconditional: n1qn1 only seeds warmZm from it, but a seed
  # built from the wrong objective is still refused rather than silently used.
  for (inner in c("trust", "n1qn1")) {
    expect_error(
      .nlmixr(model, data, "focei", control = .ctl("conditional", inner)),
      "likelihood contributions",
      info = inner
    )
  }
  # ...and the contribution alone does not disturb the default curvature
  expect_true(is.finite(.nlmixr(model, data, "focei", control = .ctl("focei", "trust"))$objf))
})
