...
test_that("conditional curvature requires a compatible optimizer", {
  ...
  expect_no_error(foceiControl(fast = TRUE, innerOpt = "n1qn1",
    innerHessian = "conditional", warm = "calc"))
})
...
test_that("conditional curvature is refused with a likelihood contribution", {
  ...
  .ctl <- function(curvature, inner) {
    foceiControl(fast = TRUE, innerOpt = inner, innerHessian = curvature,
  ...
  for (inner in c("trust", "n1qn1")) {
    expect_error(.nlmixr(model, data, "focei", control = .ctl("conditional", inner)),
                 "likelihood contributions", info = inner)
  }
