...
nmTest({
  ...
  .cens978DatCensored <- rbind(.cens978Dat, data.frame(ID = 1:10, Time = 1.5, DV = 50, cens = 1))
  .cens978DatCensored <- .cens978DatCensored[order(.cens978DatCensored$ID, .cens978DatCensored$Time), ]
  ...
  .cens978DatM2 <- rbind(.cens978Dat, data.frame(ID = 1:10, Time = 1.5, DV = 50, cens = 0))
  ...
  test_that("est='npag' residual moment excludes M3/M4 rows from the moment (#978)", {
    .ctl <- npagControl(points = 128L, cycles = 20L, gammaOptimize = FALSE, calcTables = FALSE, seed = 1L)
    ...
    expect_equal(as.numeric(f.cens$theta[["prop.sd"]]), as.numeric(f.base$theta[["prop.sd"]]), tolerance = 0.05)
  ...
  test_that("est='npb' residual moment excludes M3/M4 rows from the moment (#978)", {
    .ctl <- npbControl(points = 128L, burnin = 200L, nsamp = 200L, calcTables = FALSE, seed = 1L)
    f.base <- suppressMessages(suppressWarnings(
