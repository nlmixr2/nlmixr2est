...
nmTest({
  test_that("vaeCovariates discovers subject-level covariates with fit rules", {
    ...
    expect_equal(res$covariate, c("WT_power", "WT_lin", "WT_hockeyLow", "WT_hockeyHi", "SEX"))
  ...
  hkData <- function(wt = c(50, 60, 70, 80, 90, 100, 110, 120, 130, 140)) {
    data.frame(
      ID = rep(seq_along(wt), each = 2),
      TIME = rep(0:1, length(wt)),
      DV = seq_len(2 * length(wt)),
      WT = rep(wt, each = 2)
    )
  }
  ...
  test_that("the arms partition the subjects and sum to the lin column", {
    ...
    m <- s$covMat
    expect_equal(unname(m[, "WT_hockeyLow"] + m[, "WT_hockeyHi"]), unname(m[, "WT_lin"]))
  ...
  test_that("hockey is searched by default and can be turned off", {
    ...
    expect_false(any(grepl("hockey", vaeCovariates(hkData(), shapes = c("power", "lin"))$covariate)))
  ...
  test_that("a knot with (almost) nothing on one side drops hockey", {
    ...
    expect_warning(
      res <- vaeCovariates(hkData(), shapes = c("lin", "hockey"), covCenter = c(WT = 200)),
      "hockey skipped"
    )
    expect_equal(res$covariate, "WT_lin")
    ## a knot below every subject is the same failure on the other arm
    expect_warning(vaeCovariates(hkData(), shapes = c("lin", "hockey"), covCenter = c(WT = 10)), "hockey skipped")
    ## asking for hockey ALONE falls back to the linear family rather than
    ## dropping the covariate from the search entirely
    expect_warning(res <- vaeCovariates(hkData(), shapes = "hockey", covCenter = c(WT = 200)), "hockey skipped")
    expect_equal(res$covariate, "WT_lin")
  ...
  test_that("an empty arm is dropped even when catCutoff admits it", {
    ...
    expect_warning(
      res <- vaeCovariates(hkData(), shapes = "hockey", covCenter = c(WT = 200), catCutoff = 0),
      "hockey skipped"
    )
    expect_equal(res$covariate, "WT_lin")
    ## catCutoff still sets the threshold above that floor: 10 subjects, 2 below
    ## a knot of 70 -- kept at 20%, dropped at 30%
    expect_silent(vaeCovariates(hkData(), shapes = c("lin", "hockey"), covCenter = c(WT = 70), catCutoff = 0.2))
    expect_warning(
      vaeCovariates(hkData(), shapes = c("lin", "hockey"), covCenter = c(WT = 70), catCutoff = 0.3),
      "hockey skipped"
    )
  })
  ...
  test_that("a list-format shapes= may ask for hockey", {
    ...
    expect_equal(res$shape, c("lin", "hockeyLow", "hockeyHi"))
    res <- vaeCovariates(hkData(), shapes = list(list(var = "ka", covar = "wt", shapes = "hockey")))
  ...
  test_that("the hockey runInfo note fits on one line", {
    ...
    d <- hkData()
    .cov <- .vaeCovariateSearch(d, unique(d$ID), .vaeResolveShapes(c("lin", "hockey"))$rules, covCenter = c(WT = 200))
    expect_equal(.cov$hockeyDrop, "WT")
    .pre <- "<5% of subjects one side of knot, hockey skipped: "
    expect_lte(nchar(paste0(.pre, .vaeTruncList(.cov$hockeyDrop, prefix = .pre))), 75L)
  })
  ...
  test_that("covCenter overrides the centering statistic per covariate", {
    ...
    expect_equal(unique(vaeCovariates(d, covCenter = c(age = 40))$center), stats::median(c(70, 80, 60, 75)))
  })
  ...
  test_that("a non-0/1 two-level covariate becomes an indicator", {
    d <- data.frame(
      ...
      time = rep(0:2, 4),
      dv = 1:12,
      grp = rep(c(1, 2, 1, 2), each = 3)
    ) # 2 levels, but not 0/1
    res <- vaeCovariates(d)
  ...
  test_that("levels below catCutoff are lumped with the reference", {
    ...
    expect_equal(vaeCovariates(d, catCutoff = 0)$covariate, c("RACE_B", "RACE_A"))
  })
  ...
  test_that("vaeCovariates matches what .vaeDataPrep explores (theo_sd)", {
    ...
    prep <- suppressWarnings(.vaeDataPrep(rxode2::assertRxUi(mod), nlmixr2data::theo_sd, vaeControl()))
  ...
  test_that("vaeCovariates returns zero rows when nothing qualifies", {
    ...
    expect_equal(names(res), c("covariate", "raw", "shape", "level", "group", "block", "cluster", "type", "center"))
  })
...
nmTest({
  .mk <- function(extra) {
    cbind(data.frame(id = rep(1:6, each = 3), time = rep(0:2, 6), dv = 1:18), extra)
  }
  ...
  test_that("a covariate missing for any subject is excluded, not crashed on", {
    ...
    d2 <- .mk(data.frame(wt = rep(c(70, 80, NA, 90, 75, 65), each = 3), age = rep(c(30, 40, 50, 60, 35, 45), each = 3)))
  ...
  test_that("a covariate carrying no information yields no column", {
    ...
    expect_equal(
      nrow(vaeCovariates(.mk(data.frame(
        rc = rep("A", 18),
        stringsAsFactors = FALSE
      )))),
      0L
  ...
  test_that("a shape suffix cannot be confused with a same-named data column", {
    ...
    d <- .mk(data.frame(wt = rep(c(70, 80, 60, 90, 75, 65), each = 3), wt_lin = rep(c(0, 1, 0, 1, 0, 1), each = 3)))
    res <- vaeCovariates(d)
