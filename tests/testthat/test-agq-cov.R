...
nmTest({
  ...
  .agq_cov_1eta <- function() {
    # neta == 1: the commonest AGQ case
    ini({
      tka <- log(1.5); tcl <- log(2.7); tv <- log(31.5)
      eta.cl ~ 0.3
      add.sd <- 0.7
  ...
  test_that("the AGQ branch with a single node reproduces the FOCEI branch exactly", {
    ...
    E <- .foceiAnalyticSolveSubjectFD3(am, c(thv, setNames(ehat, paste0("ETA_", 1:3, "_"))), s1, obs$TIME, tol = tol)
    skip_if(is.null(E))
    ...
    args <- list(
      ...
      nsg = 1L,
      sgVar = ef$sgVar,
      omd = .omegaVarCovDeriv(Om, .foceiOmegaPairs(Om, ui$iniDf)),
      ndir = 3L,
      dirTh = 1:3,
      Oi = solve(Om)
    )
    ...
    g1 <- .agq(3L, 1L)
    Ragq <- do.call(.foceiAnalyticSubjectR, c(args, list(qx = g1$x, qw = g1$w, solveNode = function(etak) E)))
  ...
  test_that("AGQ declines (message + NULL) for every scope the node terms do not cover", {
    ...
    fit <- suppressMessages(nlmixr(
      ...
      nlmixr2data::theo_sd,
      "agq",
      agqControl(print = 0L, covMethod = "", nAGQ = 2L, interaction = 0L)
    ))
