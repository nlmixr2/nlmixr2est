nmTest({
  ...
  test_that("an mtime right hand side that is not a constant is carried", {
    ...
    .ui <- rxode2::rxUiDecompress(.mk(c(
      .base,
      "mtime(t5) <- 5",
      "mtime(t8) <- 8",
      "kmult <- ifelse(t < t5, 1.0, ifelse(t < t8, 2.0, 3.0))",
      .tail
    ))())
    ...
    .ui <- rxode2::rxUiDecompress(.mk(c(
      ...
      "mtime(ta) <- 5",
      "mtime(tb) <- ta + 3",
      "kmult <- ifelse(t < ta, 1.0, ifelse(t < tb, 2.0, 3.0))",
      .tail
    ))())
    ...
    .ui <- rxode2::rxUiDecompress(.mk(
      c(.base, "mtime(tsw5) <- exp(tsw)", "kmult <- ifelse(t < tsw5, 1.0, 2.0)", .tail),
      useTsw = TRUE
    )())
    ...
    .ui <- rxode2::rxUiDecompress(.mk(c(.base, "mtime(tw) <- WT / 10", "kmult <- ifelse(t < tw, 1.0, 2.0)", .tail))())
    ...
    expect_equal(
      unique(
        as.data.frame(
          rxode2::rxSolve(.ref, rxode2::et(amt = 1) |> rxode2::et(0:5))
        )$tb
      ),
      3
    )
    ...
    .ui <- rxode2::rxUiDecompress(.mk(c(
      ...
      "mtime(tx) <- t1",
      "t1 <- 20",
      "kmult <- ifelse(t < tx, 1.0, 2.0) * t1 / 20",
      .tail
    ))())
  ...
  test_that("an emitted mtime() line resolves in every generated model", {
    ...
    .resolves(.norm(rxode2::rxUiDecompress(.mkTheta()())$saemModelPred$predOnly), "saemModelPred")
    .resolves(.norm(rxode2::rxUiDecompress(.mkTheta()())$nlmRxModel$predOnly), "nlmRxModel")
    .resolves(.norm(rxode2::rxUiDecompress(.mkTheta()())$nlsRxModel$predOnly), "nlsRxModel")
  ...
  test_that("mtime() does not cost the table its ADDL doses", {
    ...
    .d <- do.call(
      ...
      lapply(1:3, function(.i) {
        rbind(
          data.frame(ID = .i, TIME = 0, DV = NA_real_, AMT = 100, EVID = 1, CMT = 1, II = 12, ADDL = 2),
          data.frame(ID = .i, TIME = seq(1, 36, by = 4), DV = 1, AMT = 0, EVID = 0, CMT = 2, II = 0, ADDL = 0)
        )
      })
    )
    .ctl <- foceiControl(print = 0, maxOuterIterations = 0, maxInnerIterations = 0, covMethod = "", calcTables = TRUE)
    .n <- vapply(
      c(TRUE, FALSE),
      function(.u) {
        vapply(
  ...
  test_that("a moving boundary: mtime() sensitivities match the in-place switch", {
    ...
    .sens <- function(useMtime, h = 1e-4) {
      ...
      # inner.cpp reads
      if (useMtime) {
        expect_false("tsw5" %in% rxode2::rxModelVars(.inner)$lhs)
      }
      .at <- function(.e) {
        .p <- stats::setNames(c(.th, .e), c(paste0("THETA[", 1:5, "]"), "ETA[1]"))
        .s <- suppressWarnings(
          rxode2::rxSolve(.inner, .p, .ev, returnType = "data.frame", atol = 1e-10, rtol = 1e-10)
        )
