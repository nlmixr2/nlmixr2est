# === addCovariate
test_that("adding non-categorical covariate to funstring with log-transformation", {
...
test_that("adding non-categorical covariate to funstring without log-transformation", {
...
test_that("adding categorical covariate to funstring with log-transformation", {
  funstring <- "ka <- exp(tka + eta.ka)"
  varName <- "ka"
...
test_that("adding categorical covariate to funstring without log-transformation", {
  funstring <- "ka <- tka + eta.ka"
  varName <- "ka"
  covariate <- "cov_factor_1_ka*factor_1+cov_factor_2_ka*factor_2+cov_factor_3_ka*factor_3"
  theta <- list("tka")
...
test_that("removing non-categorical covariate from funstring with log-transformation", {
...
test_that("removing non-categorical covariate from funstring without log-transformation", {
...
test_that("removing categorical covariate from funstring with log-transformation", {
...
test_that("removing categorical covariate from funstring without log-transformation", {
...
test_that("normalize non-categorical covariates using mean and norm type mul", {
  data <- theo_sd
  covariate <- "WT"
  normOp <- `*`
  ...
  expect_equal(covNameMod1, covNameMod2)
  expect_equal(covNames1, covNames2)
})

test_that("normalize non-categorical covariates using mean and norm type div", {
  data <- theo_sd
  covariate <- "WT"
  normOp <- `/`
  normValVec <- mean(data[, "WT"])
...
test_that("normalize non-categorical covariates using mean and norm type sub", {
  data <- theo_sd
  covariate <- "WT"
  normOp <- `-`
  varName <- "ka"
...
test_that("normalize non-categorical covariates using mean and norm type add", {
  data <- theo_sd
  covariate <- "WT"
  varName <- "ka"
  normOp <- `+`
...
test_that("normalize non-categorical covariates with log-transformation using mean and norm type mul", {
  data <- theo_sd
  covariate <- "WT"
  varName <- "ka"
  normOp <- `*`
...
test_that("normalize non-categorical covariates with log-transformation using mean and norm type div", {
  data <- theo_sd
  covariate <- "WT"
  varName <- "ka"
  normOp <- `/`
...
test_that("normalize non-categorical covariates with log-transformation using mean and norm type add", {
  data <- theo_sd
  covariate <- "WT"
  varName <- "ka"
  normOp <- `+`
...
test_that("normalize (with prefactor for Cl) for non-categorical covariates with log-transformation using mean and norm type add", {
  data <- theo_sd
  covariate <- "WT"
  varName <- "cl"
  normOp <- `+`
...
test_that("categorical covariates without normalization", {
  data <- theo_sd
  ...
  expect_equal(dat1a, dat2a)
  expect_equal(dat1b, dat2b)
  expect_equal(dat1c, dat2c)
...
# ==== makeHockeyStick
test_that("creating hockey stick variables", {
  data <- theo_sd
  covariate <- "WT"
  varName <- "ka"
  ...
  expect_equal(dat1a, dat2a)
  expect_equal(dat1b, dat2b)
  expect_equal(covModExpr1, covModExpr2)
  expect_equal(covNames1, covNames2)
})
