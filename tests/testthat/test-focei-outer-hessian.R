test_that("the subject information kernel differentiates the FOCEI objective", {
  ...
  objective <- function(p) {
    ...
    variance <- (p[2] * f)^2
    sum((y - f)^2 / variance + log(variance)) + eta^2 / p[3] + log(p[3]) + log(length(y) * (1 / p[2]^2 + 2) + 1 / p[3])
  }
  for (p in list(c(0.2, 0.2, 0.2), c(1, 0.5, 0.3))) {
    ...
    information <- foceiSubjectRFR_(
      ...
      eta,
      matrix(1 / p[3]),
      array(-1 / p[3]^2, c(1, 1, 1)),
      array(2 / p[3]^3, c(1, 1, 1)),
      matrix(-1 / p[3]^2),
      1L,
      2L,
      2L,
