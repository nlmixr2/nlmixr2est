...
nmTest({
  ...
  test_that("rz = 0 leaves the correlation exactly where it was", {
    ...
    for (.r in c(-0.9, -0.5, 0, 0.3, 0.5, 0.9)) {
      expect_equal(nlmixr2est:::rxEtaDistCorFromRz_(.r, 0), .r, tolerance = 1e-12)
    }
  })
  ...
  test_that("residual latent correlation moves rho in its own direction", {
    ...
    expect_gt(nlmixr2est:::rxEtaDistCorFromRz_(0.3, 0.2), 0.3)
    expect_lt(nlmixr2est:::rxEtaDistCorFromRz_(0.3, -0.2), 0.3)
    expect_gt(nlmixr2est:::rxEtaDistCorFromRz_(-0.3, 0.2), -0.3)
    ## and it is monotone in rz
    .v <- vapply(seq(-0.9, 0.9, by = 0.1),
                 function(z) nlmixr2est:::rxEtaDistCorFromRz_(0.4, z), numeric(1))
  ...
  test_that("the update matches the S_n = L S_z L' definition", {
    ...
    for (.r in c(-0.7, -0.2, 0, 0.25, 0.6, 0.85)) {
      for (.z in c(-0.6, -0.1, 0, 0.15, 0.5)) {
        expect_equal(nlmixr2est:::rxEtaDistCorFromRz_(.r, .z), .ref(.r, .z),
                     tolerance = 1e-10,
  ...
  test_that("the result is clamped where the copula stays identified", {
    ...
    expect_lte(nlmixr2est:::rxEtaDistCorFromRz_(0.98, 0.99), 0.99)
    expect_gte(nlmixr2est:::rxEtaDistCorFromRz_(-0.98, -0.99), -0.99)
    expect_true(is.finite(nlmixr2est:::rxEtaDistCorFromRz_(0.999999, 0.999999)))
    ## a non-finite statistic is treated as "no information", i.e. no move
    expect_equal(nlmixr2est:::rxEtaDistCorFromRz_(0.4, NaN), 0.4, tolerance = 1e-12)
    expect_equal(nlmixr2est:::rxEtaDistCorFromRz_(0.4, NA_real_), 0.4, tolerance = 1e-12)
  })
  ...
  test_that("it is NOT the product-moment correlation of the combined latents", {
    ...
    expect_equal(nlmixr2est:::rxEtaDistCorTest_(.zj, .w(.zj, .zk)), .rho, tolerance = 0.02)
    expect_equal(nlmixr2est:::rxEtaDistCorFromRz_(.rho, 0), .rho, tolerance = 1e-12)
    ...
    expect_equal(nlmixr2est:::rxEtaDistCorFromRz_(.rho, .rzBig), .rho, tolerance = 0.02)
    ...
    .pm <- nlmixr2est:::rxEtaDistCorTest_(.zjBig, .w(.zjBig, .zk))
  ...
  test_that("the copula write has its own counter", {
    ...
    expect_true(is.numeric(nlmixr2est:::foceiEtaDistCorN_()))
    expect_gte(nlmixr2est:::foceiEtaDistCorN_(), 0)
  })
