nmTest({
  ...
  test_that(".bobyqaRetryIfStuck() retries a stuck search with a wider rhobeg", {
    ...
    .stuckRet <- list(par = .par, fval = .fn(.par), feval = 7L, ierr = 0L, msg = "Normal exit from bobyqa")
  ...
  test_that(".bobyqaRetryIfStuck() leaves an already-moved result alone", {
    skip_on_cran()
    .movedRet <- list(par = c(0.5, -0.3, 0.2), fval = 0, feval = 20L, ierr = 0L, msg = "Normal exit from bobyqa")
