nmTest({
  ...
  test_that("generated models do not re-emit splitBolus()", {
    ...
    for (.n in c("inner", "predOnly", "predNoLhs")) {
      .noSplit(.fm[[.n]])
    }
  })

  test_that("splitBolus() doses are split exactly once during estimation", {
    .ctl <- foceiControl(maxOuterIterations = 0, maxInnerIterations = 20, covMethod = "", print = 0, calcTables = FALSE)
  ...
  .mkInterp <- function(interp) {
    .bdy <- c(
      if (!is.null(interp)) paste0(interp, "(WT)"),
      "ka <- exp(tka)",
      "cl <- exp(tcl + 0.05 * WT)",
      "v <- exp(tv)",
      "d/dt(depot) <- -ka * depot",
  ...
  test_that("nlm/nls honor the model's covariate interpolation", {
    ...
    expect_false(isTRUE(all.equal(.objf("nlm", .nctl, .mkInterp("locf")), .objf("nlm", .nctl, .mkInterp("nocb")))))
    .sctl <- nlsControl(maxiter = 1)
    expect_false(isTRUE(all.equal(.objf("nls", .sctl, .mkInterp("locf")), .objf("nls", .sctl, .mkInterp("nocb")))))
