test_that("M2/M3/M4 outer curvature follows the censored fit objective", {
  ...
  base <- data.frame(
    ID = rep(1:2, each = 5),
    TIME = rep(1:5, 2),
    DV = c(1.8, 1.6, 1.4, 1.2, 0.9, 0.9, 0.8, 0.75, 0.7, 0.6),
    AMT = 0,
    EVID = 0
  )
  for (errorType in c("add", "prop")) {
    for (kind in c("M2", "M3", "M4")) {
      for (family in c("FOCEI", "FOCE", "FOCE+", "AGQ")) {
        data <- base
        data$CENS <- 0L
        if (kind != "M2") {
          data$CENS <- ifelse(data$DV < 0.8, 1L, ifelse(data$DV > 1.7, -1L, 0L))
          data$DV[data$CENS == 1] <- 0.8
          data$DV[data$CENS == -1] <- 1.7
        }
        if (kind != "M3") {
          data$LIMIT <- ifelse(data$CENS == -1, 3, 0)
        }
        observed <- NULL
        optimizer <- function(par, fn, gr, lower, upper, control) {
          ...
          step <- 2e-3
          f0 <- fn(par)
          fp <- fn(par + step * direction)
          fm <- fn(par - step * direction)
          observed <<- list(
            hessian = hessian,
            reference = (reference + t(reference)) / 2,
            drift = abs(after - value),
            slope = sum(gradient * direction),
            slopeFd = (fp - fm) / (2 * step),
        ...
        .nlmixr(
          ...
          control = foceiControl(
            ...
            innerOpt = "n1qn1",
            censOption = "gauss",
            interaction = !(family %in% c("FOCE", "FOCE+")),
            foce = if (family == "FOCE+") "foce+" else "nonmem",
            nAGQ = if (family == "AGQ") 3L else 0L,
            print = 0,
            covMethod = "",
            calcTables = FALSE,
