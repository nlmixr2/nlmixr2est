nmTest({
  ...
  test_that(".nlmixr2CholPartial validates only the finite submatrix", {
    ...
    expect_false(inherits(.nlmixr2CholPartial(covm), "try-error"))

    covm2 <- covm
    covm2[2, ] <- NA_real_
    covm2[, 2] <- NA_real_
    # NA row/col for one parameter, rest still positive-definite: still valid
    expect_false(inherits(.nlmixr2CholPartial(covm2), "try-error"))

    covm3 <- covm
    covm3[1, 1] <- -1 # not positive-definite even ignoring NA
    expect_true(inherits(.nlmixr2CholPartial(covm3), "try-error"))

    covm4 <- matrix(NA_real_, 2, 2)
    expect_true(inherits(.nlmixr2CholPartial(covm4), "try-error"))
  })
})
