...
test_that("foceiLikRun() returns per-id log-likelihoods on the right scale", {
  ...
  llData <- foceiLikRun(h$initPar, eta0, type = "data")
...
test_that("foceiLikRun(type='overall') adds exactly the Gaussian eta prior", {
  skip_on_cran()
  d <- .foceiLikData()
  h <- foceiLikLoad(.foceiLikMod, d, "focei")
  on.exit(foceiLikUnload(), add = TRUE)
  ...
  for (etaVal in list(rep(0, 4), c(-0.3, 0.1, 0.25, 0.4))) {
    ...
    llD <- foceiLikRun(h$initPar, eta, type = "data")
    llO <- foceiLikRun(h$initPar, eta, type = "overall")
    # overall - data must be the fully normalized N(0, Omega) log density.  The
...
test_that("foceiLikRun() is invariant to the thread count", {
  ...
  llS <- foceiLikRun(h$initPar, eta, type = "overall", cores = 1L)
  llP <- foceiLikRun(h$initPar, eta, type = "overall", cores = 4L)
...
test_that("each likelihood type runs; interaction only matters away from eta=0", {
  ...
  res <- lapply(c("focei", "focep", "foce"), function(lik) {
    ...
    list(zero = foceiLikRun(h$initPar, eta0, type = "overall"),
         nz = foceiLikRun(h$initPar, etaNz, type = "overall"))
  })
...
test_that("foceiLikRun() responds to theta changes", {
  ...
  ll0b <- foceiLikRun(h$initPar, eta0, type = "data")
