...
nmTest({
  ...
  test_that("fast=TRUE control defaults: outerOpt + derivative-free downgrade", {
    ...
    expect_equal(.foceiFastCtl(list(foceiControl()), foceiControl)$outerOptTxt,
                 "lbfgsb3c")
    expect_equal(.foceiFastCtl(list(foceiControl(outerOpt = "nlminb")), foceiControl)$outerOptTxt,
                 "nlminb")
