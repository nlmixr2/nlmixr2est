...
nmTest({
    ...
    for (i in c(1L, 5L, 9L)) {
      gfd <- fd1(
        function(z) {
          m <- mu
          m[i, 1] <- z[1]
          elboFun(m, omega, theta, logPopOmega)
        },
        mu[i, 1, drop = FALSE],
        1L
      )
      ...
      gfd <- fd1(
        function(z) {
          o <- omega
          o[i, 1] <- z[1]
          elboFun(mu, o, theta, logPopOmega)
        },
        omega[i, 1, drop = FALSE],
        1L
      )
  ...
  test_that("adviElboGrad_ gTheta is right for an ESTIMATED boxCox lambda (#949)", {
    ...
    for (p in seq_len(ntheta)) {
      ...
      expect_true(relOk(a$gTheta[p], gfd), info = paste0("theta ", p, ": analytic ", a$gTheta[p], " fd ", gfd))
    }
  ...
  test_that("adviElboGradFR_ full-rank gradient matches finite differences", {
    ...
    for (s in c(1L, 5L)) {
      gfd <- (elbo(`[<-`(mu, s, 1, mu[s, 1] + h), Lp, theta, logPopOmega) -
        elbo(`[<-`(mu, s, 1, mu[s, 1] - h), Lp, theta, logPopOmega)) /
        (2 * h)
      expect_true(relOk(a$gMu[s, 1], gfd))
    }
    for (p in seq_len(ntheta)) {
      gfd <- (elbo(mu, Lp, `[<-`(theta, p, theta[p] + h), logPopOmega) -
        elbo(mu, Lp, `[<-`(theta, p, theta[p] - h), logPopOmega)) /
        (2 * h)
      expect_true(relOk(a$gTheta[p], gfd))
    }
  })
