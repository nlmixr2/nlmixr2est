...
| D18 | RNG | Use rxode2's thread-safe **threefry** RNG, never R's RNG (not thread-safe). E-step: draw etas up front via the registered `_rxode2_rxRmvnSEXP_` so no RNG runs inside the OpenMP loop. M-step scalar RNG (`rxnorm`/`rinorm`/`rxunif`) is NOT in the `rxode2ptr.h` struct -- it must be exported through the registered function-pointer struct (CRAN policy forbids direct symbol linking), requiring an rxode2 worktree change | See `06-parallelization.md` |
