...
| D18 | RNG | Use rxode2's thread-safe **threefry** RNG, never R's RNG (not thread-safe). E-step: draw etas up front via the registered `_rxode2_rxRmvnSEXP_` so no RNG runs inside the OpenMP loop | See `06-parallelization.md` |
| D19 | M-step RNG (M1) | Pre-draw all M-step randomness (proposals + acceptance uniforms) up front via the already-exported threefry `rxRmvn` (uniforms via `pnorm` of normals); within one M-step `mu`/`Omega` are fixed so draws are iid and K=1 is a single sequential chain. No rxode2 change for M1. Defer exporting scalar RNG (`rxnorm`/`rinorm`/`rxunif`) through the `rxode2ptr.h` struct to M2 (parallel mixture chains), which also carries a cross-package build step | See `06-parallelization.md`, `05-mstep.md` |
| D20 | Sigma / residual-error updates | Reuse SAEM's residual-error update machinery for the different normal error types (additive, proportional, combined1/2, power/lambda) rather than reimplementing -- `src/saem.cpp` (`rmProp`/`rmAddProp`/`rmAddPropLam`/`handleF`/`ares`/`bres`) and `saem_fit_aux.R`. Apply the SAME formulas to the accepted MH samples. Pure additive is already done via the Eq-17 closed form (SS backed out of stored log p) | See `05-mstep.md` |
| D21 | Per-thread scalar RNG (2026-07-09) | rxode2 now exports `seedEng(ncores)` + `rxNormEng(mean, sd)` through the registered `rxode2ptr.h` struct (indices 72-73): after `setRxThreadId()`, draw a threefry normal on that thread's engine. This is the CRAN-compliant per-thread scalar RNG D18/D19 deferred -- use it for the OpenMP E-step (task #7) and the M2 parallel mixture chains, instead of the up-front rxRmvn pool where per-thread draws are cleaner | See `06-parallelization.md` |
| D22 | mu2 covariates | Covariates on mu-referenced (eta) params are estimated by generalizing the conjugate `mu` update from a mean to a weighted linear regression of accepted `theta` on the per-subject design matrix `[1, cov...]` (reduces to the mean when no covariates). Read from `ui$muRefCovariateDataFrame`; the rpem model already carries the covariate term in the solve so the E-step is unchanged. Covariate coefficients on no-eta params are fixed effects (numeric, deferred with D20) | See `03-parameter-space.md`, `05-mstep.md` |
