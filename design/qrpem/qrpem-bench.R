...
evalObj <- function(isample, qr, seed) {
  f <- suppressWarnings(suppressMessages(
    nlmixr2(
      ...
      dat,
      "impmap",
      impmapControl(print = 0L, nIter = 1L, isample = as.integer(isample), qr = qr, impSeed = as.integer(seed))
    )
  ))
  f$env$impObjTrace[1]
}
...
for (i in seq_len(nrow(res))) {
  o <- vapply(
    seq_len(nrep),
    function(r) {
      evalObj(res$N[i], res$method[i] == "qr", 1000L + r)
    },
    numeric(1)
  )
...
cat(sprintf(
  "qr beats mc at the largest N (%d): %s\n",
  max(sizes),
  res$rmse[res$method == "qr" & res$N == max(sizes)] < res$rmse[res$method == "mc" & res$N == max(sizes)]
))
...
runTrace <- function(qr, qrRefresh = TRUE) {
  f <- suppressWarnings(suppressMessages(
    nlmixr2(
      ...
      dat,
      "impmap",
      impmapControl(print = 0L, nIter = 40L, isample = 300L, nConvWindow = 0L, qr = qr, qrRefresh = qrRefresh)
    )
  ))
...
cat(sprintf(
  "max relative theta difference: %.3f%%\n",
  100 *
    max(
      abs(fixef(fq)[pars] - fixef(ff)[pars]) /
        abs(fixef(ff)[pars])
    )
))
