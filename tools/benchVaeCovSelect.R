...
runOne <- function(nGroups, shapesPer, bnbCapBits = 22) {
  ...
  got <- vaeScoreSupports_(y, X, omega, penalty, cand, polish = TRUE, group = grpArg)
  tL <- proc.time()[3] - t0

  if (bits > bnbCapBits) {
    return(list(bits = bits, tB = NA_real_, tL = tL, same = NA))
  }
  t0 <- proc.time()[3]
  ref <- vaeBestSubset_(matrix(y, ncol = 1), X, omega, FALSE, penalty, "lifo", grpArg)
  tB <- proc.time()[3] - t0
  list(bits = bits, tB = tB, tL = tL, same = identical(which(got$selected == 1L), which(ref$selected[1, ] == 1L)))
}
...
run <- function(nGroups, shapesPer) {
  ...
  if (is.na(.tB)) {
    cat(sprintf(
      "%6d %7d %6d %8.2f %10s %10.4f %10s %8s\n",
      nGroups,
      shapesPer,
      nGroups * shapesPer,
      .bits,
    ...
    cat(sprintf(
      "%6d %7d %6d %8.2f %10.4f %10.4f %10.2f %8s\n",
      nGroups,
      shapesPer,
      nGroups * shapesPer,
      .bits,
...
for (g in c(13L, 15L, 16L, 17L, 18L, 20L)) {
  run(g, 1L)
}
cat("-- two shape families per covariate (new default) --\n")
for (g in c(8L, 9L, 10L, 11L, 12L, 13L)) {
  run(g, 2L)
}
