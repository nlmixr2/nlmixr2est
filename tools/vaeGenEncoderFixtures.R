...
## ---- modules (per spec) -------------------------------------------------
lstm <- nn_lstm(input_size = xDim, hidden_size = hDim, num_layers = 1L, batch_first = TRUE)
...
encoderForward <- function() {
  packed <- nn_utils_rnn_pack_padded_sequence(dataIn, lengths, batch_first = TRUE, enforce_sorted = FALSE)
  res <- lstm(packed)
...
golden <- list(
  meta = list(
    ...
    seed = 20260709L,
    note = "diag L = exp(log_sigma); L offdiag unconstrained"
  ),
  inputs = list(
    dataIn = arr(dataIn),
    covIn = arr(covIn),
    ...
    zPop = arr(zPop),
    omega = arr(omega),
    weight_ih = arr(lstm$weight_ih_l1),
    weight_hh = arr(lstm$weight_hh_l1),
    bias_ih = arr(lstm$bias_ih_l1),
    bias_hh = arr(lstm$bias_hh_l1),
    fc_weight = arr(linear$weight),
    fc_bias = arr(linear$bias)
  ),
  forward = list(
    mu = arr(fw$mu),
    logSigma = arr(fw$logSigma),
    L = arr(fw$L),
    z = arr(fw$z),
    loss = as.numeric(arr(loss))
  ),
  grads = list(
    weight_ih = grad(lstm$weight_ih_l1),
    weight_hh = grad(lstm$weight_hh_l1),
    bias_ih = grad(lstm$bias_ih_l1),
    bias_hh = grad(lstm$bias_hh_l1),
    fc_weight = grad(linear$weight),
    fc_bias = grad(linear$bias)
  )
)
