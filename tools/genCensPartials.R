...
r2c <- function(node) {
  if (is.numeric(node)) {
    return(formatC(node, format = "f", digits = 15))
  }
  if (is.name(node)) {
    nm <- as.character(node)
    return(if (nm == "pi") "M_PI" else nm)
  }
  if (is.call(node)) {
    op <- as.character(node[[1]])
    if (length(all.vars(node)) == 0) {
      return(formatC(eval(node), format = "f", digits = 15))
    }
    if (op == "(") {
      return(sprintf("(%s)", r2c(node[[2]])))
    }
    if (op == "/" && length(node) == 3) {
      return(sprintf("(%s / _safe_zero(%s))", r2c(node[[2]]), r2c(node[[3]])))
    }
    if (op %in% c("+", "-", "*") && length(node) == 3) {
      return(sprintf("(%s %s %s)", r2c(node[[2]]), op, r2c(node[[3]])))
    }
    if (op == "-" && length(node) == 2) {
      return(sprintf("(-%s)", r2c(node[[2]])))
    }
    if (op == "^") {
      b <- node[[3]]
      if (is.numeric(b) && b == 2) {
        return(sprintf("(%s*%s)", r2c(node[[2]]), r2c(node[[2]])))
      }
    ...
    if (op == "PHIU") {
      return(sprintf("(2.0*Rf_pnorm5(M_SQRT2*(%s), 0.0, 1.0, 1, 0))", r2c(node[[2]])))
    }
    if (op == "PHIL") {
      return(sprintf("(2.0*Rf_pnorm5((-M_SQRT2)*(%s), 0.0, 1.0, 1, 0))", r2c(node[[2]])))
    }
    if (op %in% c("sqrt", "exp", "log", "erf")) {
      return(sprintf("%s(%s)", op, r2c(node[[2]])))
    }
...
genBlock <- function(m, specs, indent = "    ") {
  blk <- paste(
    vapply(
      names(specs),
      function(sp) {
        sprintf("d_%s=%s", sp, as.character(partial(rho[[m]], specs[[sp]])))
      },
      character(1)
    ),
    collapse = "\n"
  ...
  opt <- suppressMessages({
    ...
    sink(tf)
    on.exit(sink())
    r <- rxode2::rxOptExpr(blk, "c")
    sink()
    r
  })
  lines <- strsplit(opt, "\n")[[1]]
  lines <- lines[nzchar(trimws(lines)) & !grepl("====|→|optimiz|duplicate", lines)]
  lines <- stabilizeErf(lines)
  out <- character(0)
  for (ln in lines) {
    lr <- regmatches(ln, regexec("^\\s*([A-Za-z0-9_]+)\\s*[~=]\\s*(.*)$", ln))[[1]]
    if (length(lr) < 3 || !nzchar(lr[2])) {
      next
    }
    lhs <- lr[2]
    rhs <- r2c(parse(text = lr[3])[[1]])
    if (startsWith(lhs, "d_")) {
      out <- c(out, sprintf("%sout[%d] = %s;", indent, idx[[sub("^d_", "", lhs)]], rhs))
    } else {
      out <- c(out, sprintf("%sdouble %s = %s;", indent, lhs, rhs))
    }
  }
  paste(out, collapse = "\n")
}
...
B <- list(
  M2l2 = genBlock("M2l", specs2, "      "),
  M2l3 = genBlock("M2l", specs3, "        "),
  M2g2 = genBlock("M2g", specs2, "      "),
  M2g3 = genBlock("M2g", specs3, "        "),
  M42 = genBlock("M4", specs2, "    "),
  M43 = genBlock("M4", specs3, "      "),
  M32 = genBlock("M3", specs2, "    "),
  M33 = genBlock("M3", specs3, "      ")
)
