...
ref <- suppressMessages(suppressWarnings(nlmixr(
  blk,
  dat,
  "focei",
...
th <- ref$theta
em <- as.matrix(ref$eta[, -1])
om <- ref$omega
mdl <- blk |> ini(th)
for (nm in rownames(om)) {
  mdl <- do.call(ini, list(mdl, stats::setNames(list(om[nm, nm]), nm)))
}
pin <- function(inter) {
...
rI <- utils::getFromNamespace("foceiCovAnalytic", "nlmixr2est")(fI)
rF <- utils::getFromNamespace("foceiCovAnalytic", "nlmixr2est")(fF)
...
A <- rI$R[pn, pn]
B <- rF$R[pn, pn]
big <- abs(B) > 0.01 * max(abs(B))
rel <- abs(A - B) / pmax(abs(B), 1e-12)
cat("\nmax rel |FOCEI R - FOCE R| at the SAME point:", sprintf("%.3e", max(rel[big])), "\n")
w <- which(big & rel > 1e-3, arr.ind = TRUE)
if (nrow(w)) {
  cat("disagreeing entries (>0.1%) -- FOCEI assembly defect:\n")
  for (i in seq_len(nrow(w))) {
    cat(sprintf(
      "  [%-17s, %-17s] focei=%12.5g foce=%12.5g rel=%.3f\n",
      pn[w[i, 1]],
      pn[w[i, 2]],
      A[w[i, 1], w[i, 2]],
      B[w[i, 1], w[i, 2]],
  ...
  cat("FOCEI R == FOCE R at the same point -> earlier 3.8x was the convergence confound\n")
}
