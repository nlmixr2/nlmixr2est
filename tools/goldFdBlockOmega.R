...
fitF <- suppressMessages(suppressWarnings(nlmixr(
  blk,
  dat,
  "focei",
...
fitI <- suppressMessages(suppressWarnings(nlmixr(
  blk,
  dat,
  "focei",
...
psi0 <- c(as.numeric(fitF$theta[c("tka", "tcl", "tv", "add.sd")]), Om0[1, 1], Om0[2, 2], Om0[3, 2], Om0[3, 3])
np <- length(psi0)
...
if (nrow(w)) {
  cat("entries where analytic disagrees with GOLD FD (>0.3%):\n")
  for (i in seq_len(nrow(w))) {
    cat(sprintf(
      "  [%-17s, %-17s] analytic=%12.5g gold=%12.5g rel=%.3f\n",
      pn[w[i, 1]],
      pn[w[i, 2]],
      Ran[w[i, 1], w[i, 2]],
      H[w[i, 1], w[i, 2]],
  ...
  cat("analytic R MATCHES the gold FD on all significant entries\n")
}
...
if (nrow(wI)) {
  cat("FOCEI entries disagreeing with gold (>0.3%):\n")
  for (i in seq_len(nrow(wI))) {
    cat(sprintf(
      "  [%-17s, %-17s] focei=%12.5g gold=%12.5g rel=%.3f\n",
      pn[wI[i, 1]],
      pn[wI[i, 2]],
      RanI[wI[i, 1], wI[i, 2]],
      H[wI[i, 1], wI[i, 2]],
  ...
  cat("FOCEI matches gold on all significant entries\n")
}
cat(
  "\ngold eigen:",
  paste(sprintf("%.3e", eigen(H, symmetric = TRUE, only.values = TRUE)$values), collapse = " "),
  "\n"
)
cat(
  "anlyt eigen:",
  paste(sprintf("%.3e", eigen(Ran, symmetric = TRUE, only.values = TRUE)$values), collapse = " "),
  "\n"
)
