...
- Fixed `foceiControl(gradTrim=)` lower gradient clamping (`g < -gradTrim`), which previously tested
  `g < gradTrim` and could replace small gradients by `-gradTrim` when `gradTrim` was
  finite (the default `Inf` skips clamping).
