...
.adviOptimize <- function(ui, data, control, resume = NULL) {
  ...
  .fr <- if (!is.null(resume) && !is.null(resume$family)) {
    identical(resume$family, "fullRank")
  } else {
    identical(control$viFamily, "fullRank")
  }
  ...
  .res <- adviOptimize_(list(
    pointEstimate = isTRUE(control$pointEstimate),
    fr = as.integer(.fr),
    N = as.integer(N),
    theta = as.numeric(.prep$theta),
    omega = as.numeric(.prep$omega),
    omegaMat = .prep$omegaMat,
    omegaFixMat = .prep$omegaFixMat,
    perNoCor = as.numeric(control$perNoCor),
    tol = as.numeric(control$tol),
    evalElbo = as.integer(control$evalElbo),
    jacType = as.integer(.prep$jacType),
    jacRange = as.numeric(.prep$jacRange),
    klWarmup = as.integer(control$klWarmup),
    temperInit = as.numeric(control$temperInit),
    muRefThetaIdx = as.integer(.prep$muRefThetaIdx),
    thetaFix = as.logical(.prep$thetaFix),
    omegaFix = as.logical(.prep$omegaFix),
    iters = as.integer(control$iters),
    seed = as.numeric(control$seed),
    tau = as.numeric(control$tau),
    alpha = as.numeric(control$alpha),
    nMc = as.integer(control$nMc),
    cores = .cores,
    adaptEta = isTRUE(control$adaptEta),
    etaCandidates = as.numeric(control$etaCandidates),
    nAdapt = as.integer(min(control$iters, 75L)),
    parNames = .ipNames,
    iterPrintControl = control$iterPrintControl,
    xform = .ipXform,
    resume = resume
  ))
...
.adviToFit <- function(env, res) {
  ...
  .etaIdx <- stats::setNames(match(.diagRow$name, .prep$etaNames), as.character(.diagRow$neta1))
  ...
  if (!is.null(res$parHistData)) {
    .ret$parHistData <- res$parHistData
  }
  ...
  if (isTRUE(res$tolStopped)) {
    warning(
      sprintf("ELBO converged at iteration %d of %d (emviControl(tol=))", length(res$elbo), as.integer(.control$iters)),
      call. = FALSE
    )
  }
  ...
  if (length(res$etaScores) > 1L && length(.cand) > 1L && any(is.finite(res$etaScores))) {
    ...
    if (isTRUE(.sel == max(.cand))) {
      warning("step-size search hit the top of etaCandidates; consider widening", call. = FALSE)
    } else if (isTRUE(.sel == min(.cand))) {
      warning("step-size search hit the bottom of etaCandidates; consider widening", call. = FALSE)
    }
  }
  .st <- list(
    mu = res$mu,
    theta = res$theta,
    logPopOmega = res$logPopOmega,
    popOmegaMat = res$popOmegaMat,
    nbCorrel = res$nbCorrel,
    it0 = res$it0,
    sMu = res$sMu,
    sScale = res$sScale,
    sTheta = res$sTheta,
    sLpo = res$sLpo,
    seed = res$seed,
    etaScale = res$etaScale,
    family = res$family,
    pointEstimate = res$pointEstimate
  )
...
.adviInstallThetaCov <- function(fit, cov) {
  ...
  if (!is.environment(.env) || is.null(cov)) {
    return(invisible())
  }
  .env$cov <- cov
  ...
  for (.n in rownames(.pf)) {
    if (.n %in% names(.se) && "SE" %in% names(.pf)) {
      ...
      .pf[.n, "SE"] <- .s
      if ("%RSE" %in% names(.pf)) {
        .pf[.n, "%RSE"] <- abs(.s / .e) * 100
      }
      if (
        all(c("CI Lower", "CI Upper", "Back-transformed") %in% names(.pf)) &&
...
.adviFitModel <- function(env) {
  ...
  if (!is.null(.resume)) {
    if (rxode2::rxIs(.resume, "nlmixr2FitData")) {
      .resume <- .resume$env$viState
    } else if (is.environment(.resume) && exists("viState", .resume)) {
      .resume <- .resume$viState
    }
    ...
    if (
      !is.null(.resume$pointEstimate) &&
        !identical(isTRUE(.resume$pointEstimate), isTRUE(.control$pointEstimate))
    ) {
      stop(
        "cannot resume an ",
        if (isTRUE(.resume$pointEstimate)) "emvi" else "fbvi",
        " fit under est=\"",
        if (isTRUE(.control$pointEstimate)) "emvi" else "fbvi",
        "\"",
        call. = FALSE
      )
    }
