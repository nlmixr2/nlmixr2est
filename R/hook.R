...
postFinalObjectHooksAdd <- function(name, fun) {
  checkmate::assertCharacter(name, len = 1, pattern = "^[.]*[a-zA-Z]+[a-zA-Z0-9._]*$", min.chars = 1)
  checkmate::assertFunction(fun, args = "ret")
  if (exists(name, envir = .preProcessHooks)) {
    stop("nlmixr2est pre-table hook '", name, "' already exists", call. = FALSE)
  }
...
postFinalObjectHooksRm <- function(name) {
  ...
  if (exists(name, envir = .postFinalObjectHooks)) {
    ...
    warning("nlmixr2est pre-table hook '", name, "' does not exist", call. = FALSE)
    invisible(FALSE)
  }
}
...
postFinalObjectHooks <- function(name = NULL) {
  if (is.null(name)) {
    ...
    if (exists(name, envir = .postFinalObjectHooks)) {
      ...
      stop("nlmixr2est pre-final table hook '", name, "' does not exist", call. = FALSE)
    }
  }
}
...
.postFinalObjectHooksRun <- function(ret) {
  ...
  for (name in postFinalObjectHooks()) {
    ...
    .tmp <- .fun(.ret)
    if (is.null(.tmp) || length(.tmp) == 0) {} else {
      .ret <- .tmp
    }
  }
  .ret
...
preProcessHooksAdd <- function(name, fun) {
  ...
  if (exists(name, envir = .preProcessHooks)) {
    stop("nlmixr2est preprocessing hook '", name, "' already exists", call. = FALSE)
  }
...
preProcessHooksRm <- function(name) {
  ...
  if (exists(name, envir = .preProcessHooks)) {
    ...
    warning("nlmixr2est preprocessing hook '", name, "' does not exist", call. = FALSE)
    invisible(FALSE)
  }
}
...
preProcessHooks <- function(name = NULL) {
  if (is.null(name)) {
    ...
    if (exists(name, envir = .preProcessHooks)) {
      ...
      stop("nlmixr2est preprocessing hook '", name, "' does not exist", call. = FALSE)
    }
  }
}
  ...
  .ret <- .collectWarn(
    {
      ...
      for (name in preProcessHooks()) {
        ...
        if (is.null(.ret) || length(.ret) == 0) {} else if (
          checkmate::testList(.ret, max.len = 4, min.len = 1) &&
            !is.null(names(.ret)) &&
          ...
          stop(
            "nlmixr2est preprocessing hook '",
            name,
            "' must return a list with elements 'ui', 'est', 'data', and/or 'control'",
            call. = FALSE
          )
        }
      }
