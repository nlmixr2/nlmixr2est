...
.foceiNameOccurrence <- function(ui, .names) {
  ...
  .count <- function(.e, .nm) {
    if (is.name(.e)) {
      return(as.integer(identical(as.character(.e), .nm)))
    }
    if (is.call(.e)) {
      return(sum(vapply(as.list(.e)[-1L], .count, integer(1), .nm)))
    }
    0L
  }
  .isAssign <- function(.ex) {
    is.call(.ex) && length(.ex) == 3L && (identical(.ex[[1]], as.name("<-")) || identical(.ex[[1]], as.name("=")))
  }
  stats::setNames(
    vapply(
      as.character(.names),
      function(.n) {
        if (is.null(.lst)) {
          ...
          sum(vapply(
            .lst,
            function(.ex) {
              if (.isAssign(.ex)) .count(.ex[[3]], .n) else 0L
            },
            integer(1)
          ))
        }
...
#' @noRd
.foceiAnalyticDirections <- function(ini, thetaForEta, sgName, neta, iovVars = character(0), sharedEta = logical(0)) {
  thRows <- ini[!is.na(ini$ntheta), , drop = FALSE]
  thRows <- thRows[order(thRows$ntheta), , drop = FALSE]
  thStructRows <- thRows[
    !.iniIsFixed(ini, thRows$name) & !(thRows$name %in% sgName) & !(thRows$name %in% iovVars),
    ,
    drop = FALSE
  ]
  ...
  nth <- length(thStruct)
  if (nth == 0L) {
    return(.foceiAnalyticFallback("no estimated structural parameter"))
  }
  ...
  for (p in seq_len(nth)) {
    ...
    if (length(.mt) > 1L) {
      # shared by >1 eta -> summing routes NYI, fall back to FD
  ...
  .lamRows <- ini[
    ini$name %in% sgName & !is.na(ini$err) & ini$err %in% c("boxCox", "yeoJohnson") & !.iniIsFixed(ini, ini$name),
    ,
    drop = FALSE
  ]
  lamNames <- character(0)
  lamDir <- integer(0)
  if (nrow(.lamRows) > 0L) {
    .lamRows <- .lamRows[order(.lamRows$ntheta), , drop = FALSE]
    for (r in seq_len(nrow(.lamRows))) {
      dirs <- c(dirs, paste0("THETA_", .lamRows$ntheta[r], "_"))
      lamNames <- c(lamNames, .lamRows$name[r])
      lamDir <- c(lamDir, length(dirs))
    }
    thStruct <- c(thStruct, lamNames)
    dirTh <- c(dirTh, lamDir)
    nth <- nth + length(lamNames)
  }
  ...
  list(
    thStruct = thStruct,
    thStructRows = thStructRows,
    dirs = dirs,
    dirTh = dirTh,
    ndir = length(dirs),
    nth = nth,
    dirsCov = dirsCov,
    ndirCov = length(dirsCov),
    sgName = .sgRows$name,
    lamNames = lamNames,
    lamDir = lamDir,
    lamIdx = lamIdx,
    dirP = c(dirTh, dirSg)
  ) # every non-Omega param -> a direction
}
...
#' @noRd
.foceiAnalyticAssembleRFR <- function(
  ui,
  th,
  ebes,
  ids,
  ...
  if (!is.null(startedEnv)) {
    assign(".analyticStarted", TRUE, startedEnv)
  }
  ...
  if (am$ndir != (ndirCov - length(.sigDirs))) {
    return(.foceiAnalyticFallback(paste0(
      "an augmented-model direction mismatch (",
      am$ndir,
      "/",
      ndirCov - length(.sigDirs),
      ")"
    )))
  }
  ...
  if (.foce) {
    ...
    nsub <- length(ids)
    Elist <- vector("list", nsub)
    E0list <- vector("list", nsub)
    eta0list <- vector("list", nsub)
    nobsAll <- integer(nsub)
    ...
    .obsAll <- lapply(seq_len(nsub), function(i) {
    ...
    if (.rsigA && !.fp) {
      # Route A: rebuild frozen-R0 sigma slots (aR/AR) from rsig
      .ns0 <- ncol(E0all[[1L]]$Rsig)
      if (!is.null(.ns0) && .ns0 > 0L) {
        E0all <- lapply(E0all, function(.E0) .foceiAnalyticExpandSigma(.E0, .ns0, neta, NULL, NULL, .sigSel))
      }
    }
    eta0Mat <- ebes # the fit's own EBEs -- see .foce above
    .batch <- !nzchar(Sys.getenv("FOCEI_NO_FD3_BATCH"))
    .EsAll <- if (.batch) {
      .foceiAnalyticSolveAllFD3(am, th, eta0Mat, .idCode, data, .obsT, tol = solveTol, withR = FALSE, sigSel = .sigSel)
    } else {
      NULL
    }
    if (.batch && is.null(.EsAll)) {
      .batch <- FALSE
    }
    for (i in seq_len(nsub)) {
      ...
      E <- if (.batch) {
        ...
        .foceiAnalyticSolveSubjectFD3(am, c(th, setNames(eta0, etav)), s, obs$TIME, tol = solveTol, withR = FALSE)
      }
      if (is.null(E)) {
    ...
    totObs <- sum(nobsAll)
    off <- c(0L, cumsum(nobsAll))
    nd2 <- ndirCov * ndirCov
    aB <- matrix(0, totObs, ndirCov)
    aReB <- matrix(0, totObs, ndirCov)
    aRcB <- matrix(0, totObs, ndirCov)
    AB <- array(0, c(totObs, ndirCov, ndirCov))
    AReB <- array(0, c(totObs, ndirCov, ndirCov))
    ARcB <- array(0, c(totObs, ndirCov, ndirCov))
    ...
    for (i in seq_len(nsub)) {
      ...
      E0 <- E0list[[i]]
      no <- nobsAll[i]
      rows <- (off[i] + 1L):off[i + 1L]
      aB[rows, ] <- E$a
      AB[rows, , ] <- E$A
      AthB[rows, , ] <- array(E$Ath, c(no, neta, nd2))
      fB[rows] <- E$f
      yB[rows] <- E$y
      if (.fpG) {
        R0B[rows] <- E$R
        ...
        aRcB[rows, ] <- E0$aR
        ARcB[rows, , ] <- E0$AR
      } # aReB/AReB stay 0 (frozen)
      if (length(lamDir) || .hasCens) {
        s <- .byId[[as.character(.idCode[i])]]
        obs <- s[s$EVID == 0, , drop = FALSE]
        if (length(lamDir)) {
          # DV-transform chain (estimated lambda)
    ...
    if (nom > 0L) {
      for (aa in seq_len(nom)) {
        for (bb in seq_len(nom)) {
          d2OiC[,, (aa - 1L) * nom + bb] <- omd$d2Oi[[aa]][[bb]]
        }
      }
    }
  ...
  .batch <- !nzchar(Sys.getenv("FOCEI_NO_FD3_BATCH"))
  .EsAll <- if (.batch) {
    .foceiAnalyticSolveAllFD3(am, th, ebes, .idCode, data, .obsT, tol = solveTol, withR = TRUE, sigSel = .sigSel)
  } else {
    NULL
  }
  if (.batch && is.null(.EsAll)) {
  ...
  for (i in seq_len(nsub)) {
    ...
    E <- if (.batch) {
      ...
      .foceiAnalyticSolveSubjectFD3(am, c(th, setNames(ebes[i, ], etav)), s, obs$TIME, tol = solveTol, withR = TRUE)
    }
    if (is.null(E)) {
      return(.foceiAnalyticFallback("a failed subject sensitivity solve"))
    }
  ...
  nd2 <- ndirCov * ndirCov
  aB <- matrix(0, totObs, ndirCov)
  aRB <- matrix(0, totObs, ndirCov)
  AB <- array(0, c(totObs, ndirCov, ndirCov))
  ARB <- array(0, c(totObs, ndirCov, ndirCov))
  AthB <- array(0, c(totObs, neta, nd2))
  AthRB <- array(0, c(totObs, neta, nd2))
  fB <- numeric(totObs)
  yB <- numeric(totObs)
  RB <- numeric(totObs)
  ehatB <- matrix(0, nsub, neta)
  ...
  for (i in seq_len(nsub)) {
    ...
    ARB[rows, , ] <- E$AR
    AthB[rows, , ] <- array(E$Ath, c(no, neta, nd2))
    AthRB[rows, , ] <- array(E$AthR, c(no, neta, nd2))
    fB[rows] <- E$f
    yB[rows] <- E$y
    RB[rows] <- E$R
    ehatB[i, ] <- ebes[i, ]
...
#' @noRd
.foceiAnalyticAssembleR <- function(
  ui,
  th,
  ebes,
  ids,
  ...
  .obsAll <- lapply(seq_along(ids), function(i) {
    .s <- .byId[[as.character(.idCode[i])]]
    if (is.null(.s) || nrow(.s) == 0L) NULL else .s[.s$EVID == 0, , drop = FALSE]
  })
  if (any(vapply(.obsAll, is.null, logical(1L)))) {
    return(NULL)
  } # unmatched subject -> caller FD
  .obsT <- lapply(.obsAll, function(.o) .o$TIME)
  .needF0 <- .foce && identical(as.integer(foceType), 0L) && isTRUE(ef$dependsF0)
  E0List <- vector("list", length(ids))
  if (.needF0) {
    # eta=0 population solve (frozen R0), batched
  ...
  for (i in seq_along(ids)) {
    ...
    E <- if (.batch) {
      ...
      .foceiAnalyticSolveSubjectFD3(am, c(th, setNames(eta0, etav)), s, obs$TIME, tol = solveTol)
    }
    if (is.null(E)) {
      return(NULL)
    } # solve failure -> caller falls back to FD
    ...
    if (rescale) {
      ...
      for (d1 in seq_len(ndir)) {
        for (d2 in seq_len(ndir)) {
          E$A[, d1, d2] <- E$A[, d1, d2] * iovDirScale[d1] * iovDirScale[d2]
        }
      }
      for (d1 in seq_len(neta)) {
        for (d2 in seq_len(ndir)) {
          for (d3 in seq_len(ndir)) {
            # Ath is [obs,neta,ndir,ndir]
            E$Ath[, d1, d2, d3] <- E$Ath[, d1, d2, d3] * iovDirScale[d1] * iovDirScale[d2] * iovDirScale[d3]
          }
        }
      }
      if (!is.null(E0)) {
        # population sensitivities share the rescaling
  ...
  tryCatch(
    {
      ...
      if (!identical(rxode2::rxGetControl(ui, "covType", "fd"), "analytic")) {
        return(NULL)
      }
      if (!.hasRxSens()) {
      ...
      if (neta == 0L) {
        return(.foceiAnalyticFallback("no random effects"))
      }
      ...
      .omegaIov <- vapply(
        ...
        function(.v) {
          .w <- which(
            grepl(paste0("^rx\\.", gsub(".", "\\.", .v, fixed = TRUE), "\\.[0-9]+$"), .idfA$name) &
              !is.na(.idfA$neta1) &
              .idfA$neta1 == .idfA$neta2
          )
      ...
      if (length(occEtaAll) > 0L && !all(occEtaAll %in% naEta)) {
        return(NULL)
      } # sanity: occ etas must be non-mu-ref
      ...
      for (g in seq_along(iovVars)) {
        w <- iovW[g]
        oe <- occEta[[g]]
        if (length(oe) == 0L || !is.finite(w) || w <= 0) {
          # unidentified/zero IOV -> FD
      ...
      if (is.null(.dir)) {
        return(.foceiAnalyticFallback("an unresolvable direction set"))
      }
      ...
      if (as.integer(rxode2::rxGetControl(ui, "nAGQ", 1L)) > 1L) {
        ...
        .why <- if (isTRUE(ef$foceiOnly)) {
          "a general or multi-endpoint residual variance"
        } else if (.hasCensD) {
          "censored observations"
        } else if (length(iovVars) > 0L) {
          "inter-occasion variability (IOV)"
        } else if (isTRUE(is.finite(.agqLo)) || isTRUE(is.finite(.agqHi))) {
          "a finite agqLow/agqHi node clamp"
        } else {
          NULL
        }
        if (!is.null(.why)) {
      ...
      if (is.null(.R0)) {
        return(NULL)
      } # inversion failed -> FD, and do NOT leave a stale .analyticCov
...
.foceiAnalyticSolveTol <- function(ui) {
  ...
  if (!is.null(.user) && is.finite(.user) && .user > 0) {
    return(.user)
  }
...
.omegaVarCovDeriv <- function(Om, pairs, iov = NULL) {
  ...
  if (nOrd > 0L) {
    for (a in seq_len(nOrd)) {
      ...
      for (b in seq_len(nOrd)) {
        d2Oi[[a]][[b]] <- d$d2OmegaInv[[ordIdx[a]]][[ordIdx[b]]]
      }
    }
...
.foceiAnalyticErrFull <- function(ui) {
  ...
  if (nrow(er) == 0L) {
    ...
    if (length(.tok) == 0L || !all(.tok %in% .known) || !all(.trans == "untransformed")) {
    ...
    return(list(
      ...
      focePlus = NULL,
      foceiOnly = TRUE,
      canVanish = !.hasAddFloor,
      dependsF0 = !all(.tok == "add"),
      estLam = .estLam,
  ...
  .isAddProp <- !.multiEndpoint &&
    all(.trans == "untransformed") &&
    all(er$err %in% c("add", "prop")) &&
    !identical(addPr, "combined1")
  ...
  } # pure d(rho)/d(sigma): first-derivative (outer-gradient) sigma term
  pair <- list()
  for (i in seq_along(sgVar)) {
    for (j in i:length(sgVar)) {
      a <- sgVar[i]
      b <- sgVar[j]
      pair[[paste0(a, b)]] <- list(
        rss = DD(rhoE, a, b),
        rfss = DD(rhoE, "f", a, b),
        pss = DD(pE, a, b),
        qss = DD(qE, a, b),
  ...
  } # pure d(rho)/d(sigma) for the FOCE first-derivative sigma term
  pairF <- list()
  for (i in seq_along(sgVar)) {
    for (j in i:length(sgVar)) {
      a <- sgVar[i]
      b <- sgVar[j]
      pairF[[paste0(a, b)]] <- list(
        rss = DD(rho0E, a, b),
        rfss = DD(rho0E, "f", a, b),
        qss = DD(q0E, a, b),
        pssF = DD(pF0E, a, b)
  ...
  f0F <- list(
    ...
    qf0f0 = DD(q0E, "f0", "f0"),
    pFf0 = DD(pF0E, "f0"),
    pFf0f0 = DD(pF0E, "f0", "f0"),
    rhof0 = DD(rho0E, "f0"),
    rhof0f0 = DD(rho0E, "f0", "f0")
  )
  perf0F <- list()
  for (s in sgVar) {
    perf0F[[s]] <- list(qf0s = DD(q0E, "f0", s), pFf0s = DD(pF0E, "f0", s), rhof0s = DD(rho0E, "f0", s))
  }
...
.foceiAnalyticCols <- function(dirs, fDirs, P2, P2r, sigTh) {
  sigP2 <- if (length(sigTh)) {
    do.call(
      rbind,
      lapply(seq_along(sigTh), function(.a) {
        data.frame(a = .a, b = seq_along(sigTh)[seq_along(sigTh) >= .a])
      })
    )
  } else {
    NULL
  ...
  list(
    f1 = paste0("rx_f1_", fDirs),
    fDirIdx = match(fDirs, dirs),
...
.foceiAnalyticEvents <- function(am, data) {
  ...
  if (is.null(.fp)) {
    return(data)
  } # cannot fingerprint -> translate fresh (no cache)
  ...
  if (is.null(.et)) {
    ...
    if (length(ls(.foceiAnalyticEtCache, all.names = TRUE)) >= 256L) {
...
.foceiAnalyticCovDirMap <- function(ui, fDirs) {
  ...
  .grab <- function(df, scaleCol) {
    if (
      is.null(df) ||
        !is.data.frame(df) ||
        nrow(df) == 0L ||
        !all(c("theta", "covariateParameter", scaleCol) %in% names(df))
    ) {
      return(NULL)
    }
    data.frame(
      theta = as.character(df$theta),
      coef = as.character(df$covariateParameter),
      scale = as.character(df[[scaleCol]]),
  ...
  for (.r in seq_len(nrow(.cov))) {
    ...
    if (!is.na(.etN)) {
      ...
      .snt <- .ini$ntheta[.ini$name == .thN]
      if (length(.snt) != 1L || is.na(.snt)) {
        next
      }
      .ed <- paste0("THETA_", .snt, "_")
      if (!(.ed %in% fDirs)) {
        next
      }
      if (!identical(.to[[.thN]], 1L)) next # structural theta shared -> scaling not exact
    }
    .nt <- .ini$ntheta[.ini$name == .cov$coef[.r]]
    if (length(.nt) != 1L || is.na(.nt)) {
      next
    }
    .cd <- paste0("THETA_", .nt, "_")
    if (!(.cd %in% fDirs) || .cd %in% .dup) {
      next
    }
...
.foceiAnalyticAugModelDirs <- function(ui, dirs, order = 2L) {
  ...
  .key <- tryCatch(
    paste0(
      rxUiGet.foceiModelDigest(list(ui)),
      "|",
      paste(dirs, collapse = ","),
      "|o",
      order, # sensitivity tier -> distinct cached model
      "|sk",
      Sys.getenv("FOCEI_NO_SIGMA_SKIP"), # skip flag -> distinct cached model
      # subject-constant covariate set -> which covariate directions get eta-scaling reuse
      "|cc",
      paste(sort(tryCatch(rxode2::rxGetControl(ui, "foceiConstCovs", NULL), error = function(e) NULL)), collapse = ",")
    ),
    error = function(e) NULL
  )
  if (!is.null(.key)) {
    .hit <- get0(.key, envir = .foceiAnalyticAugCache, inherits = FALSE)
  ...
  .res <- tryCatch(
    {
      ...
      .erN <- ui$iniDf$ntheta[!is.na(ui$iniDf$err) & !(ui$iniDf$err %in% c("boxCox", "yeoJohnson"))]
      .sigDirs <- if (!nzchar(Sys.getenv("FOCEI_NO_SIGMA_SKIP"))) {
        intersect(dirs, paste0("THETA_", .erN, "_"))
      } else {
        character(0)
      }
      ...
      if (length(.covMap)) {
        ...
        .covMap <- .covMap[vapply(
          .covMap,
          function(.m) {
            .v <- all.vars(parse(text = .m$scale)[[1]])
            length(.v) > 0L && all(.v %in% .constCovs)
          },
          logical(1)
        )]
      }
      ...
      .covSub <- function(.d) {
        .m <- .covMap[[.d]]
        if (is.null(.m)) list(d = .d, s = NULL) else list(d = .m$etaDir, s = .m$scale)
      }
      .emitScaled1 <- function(.pre, .suf, .cd) {
        paste0(.pre, .cd, .suf, "=(", .covMap[[.cd]]$scale, ")*", .pre, .covMap[[.cd]]$etaDir, .suf)
      }
      .emitPair <- function(.pre, .i, .j, .symFn, .ord) {
        .si <- .covSub(.i)
        .sj <- .covSub(.j)
        if (is.null(.si$s) && is.null(.sj$s)) {
          return(paste0(.pre, .i, "_", .j, "=", .symFn(.i, .j)))
        }
        .a <- .si$d
        .b <- .sj$d
        if (match(.a, .ord) > match(.b, .ord)) {
          .t <- .a
          .a <- .b
          .b <- .t
        }
      ...
      .fL1 <- c(
        vapply(.mfDirs, function(.p) paste0("rx_f1_", .p, "=", .toRx(.g1(.pred, .p))), character(1)),
      ...
      .fL2 <- c(
        vapply(
          which(.p2mod),
          function(.r) .emitPair("rx_f2_", .P2$i[.r], .P2$j[.r], function(.a, .b) .toRx(.g2(.pred, .a, .b)), .fDirs),
          character(1)
        ),
      ...
      if (!is.null(.rvar)) {
        .rL1 <- c(
          vapply(
            dirs[!(dirs %in% .covDirs)],
            function(.p) paste0("rx_rvar1_", .p, "=", .toRx(.g1(.rvar, .p))),
            character(1)
          ),
        ...
        .rL2 <- c(
          vapply(
            which(.pr2mod),
            function(.r) {
              .emitPair("rx_rvar2_", .P2r$i[.r], .P2r$j[.r], function(.a, .b) .toRx(.g2(.rvar, .a, .b)), dirs)
            },
            character(1)
          ),
      ...
      if (!is.null(.rvar)) {
        ...
        .frTh <- suppressWarnings(unique(as.integer(sub(
          "THETA_([0-9]+)_",
          "\\1",
        ...
        .li <- ui$iniDf
        .lamTh <- .li$ntheta[!is.na(.li$ntheta) & !is.na(.li$err) & !.li$fix & .li$err %in% c("boxCox", "yeoJohnson")]
        .sigTh <- sort(union(intersect(.erTh, .frTh), .lamTh))
        for (.n in .sigTh) {
          .sg <- paste0("THETA_", .n, "_")
          .dRs <- .Dn(.rvar, .sg)
          .sigL <- c(
            .sigL,
            paste0("rx_rsig_", .n, "_=", .toRx(.dRs)),
            vapply(
              dirs[!(dirs %in% .covDirs)],
              function(.p) paste0("rx_rsig1_", .n, "_", .p, "=", .toRx(.g1(.dRs, .p))),
              character(1)
            ),
            ...
            vapply(
              .sigTh[.sigTh >= .n],
              function(.n2) {
                paste0("rx_rsig2_", .n, "_", .n2, "=", .toRx(.Dn(.dRs, paste0("THETA_", .n2, "_"))))
              },
              character(1)
            )
          )
      ...
      .baseOde <- vapply(
        .st,
        function(.x) paste0("d/dt(", .x, ")=", .toRx(get(paste0("rx__d_dt_", .x, "__"), .s))),
        character(1)
      )
      ...
      .icDone <- trimws(sub(
        "\\(0\\)=.*$",
        "",
        grep("\\(0\\)=", unlist(strsplit(as.character(c(.s1, .s2)), "\n")), value = TRUE)
      ))
      .emitIc <- function(.cmt, .expr) {
        if (identical(.toRx(.expr), "0") || .cmt %in% .icDone) {
          character(0)
        } else {
          paste0(.cmt, "(0)=", .toRx(.expr))
        }
      }
      .icL <- character(0)
      for (.x in .st) {
        ...
        for (.p in .mfDirs) {
          .icL <- c(.icL, .emitIc(paste0("rx__sens_", .x, "_BY_", .p, "__"), .Dn(.ic, .p)))
        }
        ...
        for (.r in .p2m) {
          .icL <- c(
            .icL,
            .emitIc(
              paste0("rx__sens_", .x, "_BY_", .P2$i[.r], "_BY_", .P2$j[.r], "__"),
      ...
      .mtime <- gsub("ETA\\[([0-9]+)\\]", "ETA_\\1_", gsub("THETA\\[([0-9]+)\\]", "THETA_\\1_", .mtimeLinesStr(.s)))
      ...
      list(
        augMod = .nlmixr2estRxode2(.modTxt, "rxOuter", eventSens = "jump"),
        dirs = dirs,
        ndir = length(dirs),
        fDirs = .fDirs,
        st = .st,
        P2 = .P2,
        P2r = .P2r,
        hasRvar = !is.null(.rvar),
        sigTh = .sigTh,
        hasTrans = .hasTrans,
        cols = .foceiAnalyticCols(dirs, .fDirs, .P2, .P2r, .sigTh),
        cores = .cores,
        key = .key
      )
    },
  ...
  if (!is.null(.key) && !is.null(.res)) {
    if (length(ls(.foceiAnalyticAugCache, all.names = TRUE)) >= 64L) {
      # bound retained compiled models
...
.foceiAnalyticSolveSubjectFD3 <- function(aug, params, ev, times, .fdEps = 7e-7, tol = 1e-10, withR = FALSE) {
  ...
  # Shi-differencing A.
  E0 <- .foceiAnalyticSolveFA(aug, params, ev, times, tol = tol)
  if (is.null(E0)) {
    return(NULL)
  }
  nobs <- length(E0$f)
  ...
  .etaDir <- which(grepl("^ETA_[0-9]+_$", dirs))
  neta <- length(.etaDir)
  if (neta == 0L) {
    return(NULL)
  }
  .hasR <- isTRUE(withR) && !is.null(E0$AR)
  nA <- nobs * nd * nd
  ...
  Aflat <- function(.tt) {
    E <- .foceiAnalyticSolveFA(aug, setNames(.tt, names(params)), ev, times, tol = tol)
    if (is.null(E) || !all(is.finite(E$A))) {
      return(NULL)
    }
    if (.hasR) {
      if (!all(is.finite(E$AR))) {
        return(NULL)
      }
      c(as.vector(E$A), as.vector(E$AR))
    } else {
      as.vector(E$A)
    }
  }
  Ath <- array(0, c(nobs, neta, nd, nd))
  AthR <- if (.hasR) array(0, c(nobs, neta, nd, nd)) else NULL
  for (li in seq_len(neta)) {
    ...
    if (is.null(sc$gr) || !all(is.finite(sc$gr))) {
      return(NULL)
    }
  ...
  .out <- c(
    list(f = E0$f, a = E0$a, A = E0$A, Ath = Ath),
    if (.hasR) {
      list(R = E0$R, aR = E0$aR, AR = E0$AR, AthR = AthR, Rsig = E0$Rsig, RsigDir = E0$RsigDir, Rsig2 = E0$Rsig2)
    } else {
      NULL
    },
    if (!is.null(E0$trans)) list(trans = E0$trans) else NULL
  )
  if (!all(is.finite(.out$f)) || !all(is.finite(.out$a)) || !all(is.finite(.out$A)) || !all(is.finite(.out$Ath))) {
    return(NULL)
  }
  .out
}
...
.foceiAnalyticExpandSigma <- function(E, nsig, neta, RsigDirEta, Rsig2Eta, sigSel = NULL) {
  ...
  if (!is.null(sigSel)) {
    ...
    if (!is.null(E$RsigDir)) {
      E$RsigDir <- E$RsigDir[,, sigSel, drop = FALSE]
    }
    if (!is.null(E$Rsig2)) {
      E$Rsig2 <- E$Rsig2[, sigSel, sigSel, drop = FALSE]
    }
    if (!is.null(RsigDirEta)) {
      RsigDirEta <- RsigDirEta[,,, sigSel, drop = FALSE]
    }
    if (!is.null(Rsig2Eta)) {
      Rsig2Eta <- Rsig2Eta[,, sigSel, sigSel, drop = FALSE]
    }
    nsig <- length(sigSel)
  }
  nd <- ncol(E$a)
  ...
  A <- array(0, c(no, ndc, ndc))
  A[, seq_len(nd), seq_len(nd)] <- E$A
  .hasAth <- !is.null(E$Ath) # E0 (frozen-R0 solve) has no Ath
  Ath <- if (.hasAth) {
    .Y <- array(0, c(no, neta, ndc, ndc))
    .Y[,, seq_len(nd), seq_len(nd)] <- E$Ath
    .Y
  } else {
    NULL
  }
  aR <- cbind(E$aR, E$Rsig)
  AR <- array(0, c(no, ndc, ndc))
  AR[, seq_len(nd), seq_len(nd)] <- E$AR
  .hasAthR <- !is.null(E$AthR) && !is.null(RsigDirEta) # withR: 3rd-order sigma slots too
  AthR <- if (.hasAthR) {
    .X <- array(0, c(no, neta, ndc, ndc))
    .X[,, seq_len(nd), seq_len(nd)] <- E$AthR
    .X
  } else {
    NULL
  }
  for (k in seq_len(nsig)) {
    AR[, seq_len(nd), sg[k]] <- E$RsigDir[,, k]
    AR[, sg[k], seq_len(nd)] <- E$RsigDir[,, k]
    if (.hasAthR) {
      AthR[,, seq_len(nd), sg[k]] <- RsigDirEta[,,, k]
      AthR[,, sg[k], seq_len(nd)] <- RsigDirEta[,,, k]
    }
    for (l in seq_len(nsig)) {
      AR[, sg[k], sg[l]] <- E$Rsig2[, k, l]
      if (.hasAthR) AthR[,, sg[k], sg[l]] <- Rsig2Eta[,, k, l]
    }
  }
  E$a <- a
  E$A <- A
...
#' @noRd
.foceiAnalyticSolveAllFD3 <- function(
  am,
  thv,
  ebes,
  ids,
  ...
  .rsig <- !nzchar(Sys.getenv("FOCEI_NO_RSIG")) &&
    !is.null(E0[[1L]]$Rsig) &&
    length(E0[[1L]]$Rsig) > 0L &&
    (is.null(sigSel) || length(sigSel) > 0L)
  ...
  .cdiff <- function(li, h) {
    ...
    if (is.null(Ep) || is.null(Em) || length(Ep) != nsub || length(Em) != nsub) {
      return(NULL)
    }
    list(Ep = Ep, Em = Em, h = h)
  }
  for (li in seq_len(neta)) {
    # ETA_li coordinate == ebes column li
    ...
    for (i in seq_len(nsub)) {
      d1 <- (s1$Ep[[i]]$A - s1$Em[[i]]$A) / (2 * s1$h)
      d2 <- (s2$Ep[[i]]$A - s2$Em[[i]]$A) / (2 * s2$h)
      Ath[[i]][, li, , ] <- (4 * d2 - d1) / 3
      if (withR) {
        e1 <- (s1$Ep[[i]]$AR - s1$Em[[i]]$AR) / (2 * s1$h)
        e2 <- (s2$Ep[[i]]$AR - s2$Em[[i]]$AR) / (2 * s2$h)
        AthR[[i]][, li, , ] <- (4 * e2 - e1) / 3
        if (.rsig) {
          g1 <- (s1$Ep[[i]]$RsigDir - s1$Em[[i]]$RsigDir) / (2 * s1$h)
          g2 <- (s2$Ep[[i]]$RsigDir - s2$Em[[i]]$RsigDir) / (2 * s2$h)
          RsigDirEta[[i]][, li, , ] <- (4 * g2 - g1) / 3
          k1 <- (s1$Ep[[i]]$Rsig2 - s1$Em[[i]]$Rsig2) / (2 * s1$h)
          k2 <- (s2$Ep[[i]]$Rsig2 - s2$Em[[i]]$Rsig2) / (2 * s2$h)
...
#' @noRd
.foceiAnalyticSubjectRFR <- function(
  E,
  ehat,
  Om,
  neta,
  ...
  if (identical(as.integer(interaction), 0L)) {
    return(.foceiAnalyticSubjectRfoceFR(
      E,
      ehat,
      Om,
      neta,
  ...
  rff <- 1 / R
  rfR <- res / R^2
  rRR <- 0.5 * (-1 / R^2 + 2 * res^2 / R^3)
  rffR <- -1 / R^2
  rfRR <- -2 * res / R^3
  rRRR <- 0.5 * (2 / R^3 - 6 * res^2 / R^4) # rfff = 0
  iR <- 1 / R
  iR2 <- iR^2
  iR3 <- iR^3
  iR4 <- iR^4
  ...
  for (l in ei) {
    for (m in ei) {
      Ht[l, m] <- Ht[l, m] + sum(a[, l] * a[, m] * iR + 0.5 * aR[, l] * aR[, m] * iR2)
    }
  }
  Hti <- solve(Ht)
      ...
      for (m in ei) {
        D[l, m] <- sum(
          (A[, l, s] * a[, m] + a[, l] * A[, m, s]) *
            iR -
            a[, l] * a[, m] * aR[, s] * iR2 +
            0.5 * (AR[, l, s] * aR[, m] + aR[, l] * AR[, m, s]) * iR2 -
            aR[, l] * aR[, m] * aR[, s] * iR3
        )
      }
    }
  ...
  d2HtDD <- lapply(di, function(s) {
    lapply(di, function(t) {
      ...
      for (l in ei) {
        for (m in ei) {
          D[l, m] <- sum(
            (Ath[, l, s, t] * a[, m] + A[, l, s] * A[, m, t] + A[, l, t] * A[, m, s] + a[, l] * Ath[, m, s, t]) *
              iR -
              (A[, l, s] * a[, m] + a[, l] * A[, m, s]) * aR[, t] * iR2 -
              (A[, l, t] * a[, m] + a[, l] * A[, m, t]) * aR[, s] * iR2 -
              a[, l] * a[, m] * AR[, s, t] * iR2 +
              ...
              (AR[, l, t] * aR[, m] + aR[, l] * AR[, m, t]) * aR[, s] * iR3 -
              aR[, l] * aR[, m] * AR[, s, t] * iR3 +
              3 * aR[, l] * aR[, m] * aR[, s] * aR[, t] * iR4
          )
        }
      }
      D
  ...
  dOf <- function(p) dirP[p]
  omc <- function(p) p - ndirP
  ...
  for (aa in 1:np) {
    for (bb in aa:np) {
      ...
      ld <- Cpp(aa, bb) +
        sum(.CpeRow[[aa]] * etaP[, bb]) +
        sum(.CpeRow[[bb]] * etaP[, aa]) +
...
#' @noRd
.foceiAnalyticSubjectRfoceFR <- function(
  E,
  ehat,
  Om,
  neta,
  ...
  if (.fp) {
    ...
    R0 <- E0$R
    aRe <- matrix(0, nobs, ndir)
    aRc <- E0$aR
    ARc <- E0$AR
  }
  res <- y - f
  rf <- -res / R0
  rR <- 0.5 * (1 / R0 - res^2 / R0^2)
  rff <- 1 / R0
  rfR <- res / R0^2
  rRR <- 0.5 * (-1 / R0^2 + 2 * res^2 / R0^3)
  q0 <- rf
  q1 <- 1 / R0
  iR <- 1 / R0
  iR2 <- iR^2
  ...
  Hf <- Oi
  Nf <- matrix(0, neta, ndir)
  for (l in ei) {
    for (m in ei) {
      Hf[l, m] <- Hf[l, m] + sum(q1 * a[, l] * a[, m] + q0 * A[, l, m] + rfR * a[, l] * aRe[, m])
    }
    for (d in di) {
      Nf[l, d] <- sum(q1 * a[, l] * a[, d] + q0 * A[, l, d])
    }
  }
  HfInv <- solve(Hf)
  ...
  scoreSecond <- function(l, s, t, rs, rt, rst) {
    qs <- q1 * a[, s] + rfR * rs[, s]
    qt <- q1 * a[, t] + rfR * rt[, t]
    qst <- -(a[, s] * rt[, t] + rs[, s] * a[, t]) /
      R0^2 -
      2 * res * rs[, s] * rt[, t] / R0^3 +
      q1 * A[, s, t] +
      rfR * rst[, s, t]
  ...
  for (l in ei) {
    for (m in ei) {
      Ht[l, m] <- Ht[l, m] + sum(a[, l] * a[, m] * iR)
    }
  }
  Hti <- solve(Ht)
  ARblk <- function(s, t) if (.fp) E$AR[, s, t] else rep(0, nobs) # d2R0/(dir_s dir_t), eta-block
  dHtDir <- function(s, aRvS) {
    D <- matrix(0, neta, neta)
    for (l in ei) {
      for (m in ei) {
        D[l, m] <- sum((A[, l, s] * a[, m] + a[, l] * A[, m, s]) * iR - a[, l] * a[, m] * aRvS[, s] * iR2)
      }
    }
    D
  }
  d2HtDir <- function(s, t, aRvS, aRvT, ARv) {
    D <- matrix(0, neta, neta)
    for (l in ei) {
      for (m in ei) {
        D[l, m] <- sum(
          (Ath[, l, s, t] * a[, m] + A[, l, s] * A[, m, t] + A[, l, t] * A[, m, s] + a[, l] * Ath[, m, s, t]) *
            ...
            (A[, l, t] * a[, m] + a[, l] * A[, m, t]) * aRvS[, s] * iR2 -
            a[, l] * a[, m] * ARv * iR2 +
            2 * a[, l] * a[, m] * aRvS[, s] * aRvT[, t] * iR3
        )
      }
    }
    D
  ...
  McolEBE <- function(p) {
    if (!isD(p)) {
      return(as.numeric(omd$dOi[[omc(p)]] %*% ehat))
    }
    d <- dOf(p)
    Nf[, d] + as.numeric(crossprod(ae, chQ(d)))
  }
  McolData <- function(p) {
    value <- McolEBE(p)
    if (isD(p)) {
  ...
  SmatEBE <- function(p) {
    ...
    for (l in ei) {
      for (s in di) {
        value[l, s] <- scoreSecond(l, s, dOf(p), aRe, aRc, aRe2)
      }
    }
    value
  }
  SvecEBE <- function(p, q) {
    if (!isD(p) && !isD(q)) {
      return(as.numeric(omd$d2Oi[[omc(p)]][[omc(q)]] %*% ehat))
    }
    value <- numeric(neta)
    if (isD(p) && isD(q)) {
      for (l in ei) {
  ...
  d2HtEtaP <- function(p, l) {
    ...
    d <- dOf(p)
    d2HtDir(d, l, aRc, aRe, if (.fp) E$AR[, d, l] else rep(0, nobs))
  }
  d2Ht_pp <- function(aa, bb) {
    ta <- isD(aa)
    tb <- isD(bb)
    if (ta && tb) {
      return(d2HtDir(dOf(aa), dOf(bb), aRc, aRc, ARc[, dOf(aa), dOf(bb)]))
    }
    if (!ta && !tb) {
      return(omd$d2Oi[[omc(aa)]][[omc(bb)]])
    }
  ...
  eta2 <- function(aa, bb) {
    b <- SvecEBE(aa, bb) +
      SmatEBE(aa)[, ei, drop = FALSE] %*% etaP[, bb] +
      SmatEBE(bb)[, ei, drop = FALSE] %*% etaP[, aa]
    for (l in ei) {
      b[l] <- b[l] + as.numeric(t(etaP[, aa]) %*% Tnf[l, ei, ei] %*% etaP[, bb])
    }
    as.numeric(-HfInv %*% b)
  }
  .CpeRow <- lapply(1:np, function(p) vapply(ei, function(l) Cpe(p, l), numeric(1)))
  ...
  for (aa in 1:np) {
    for (bb in aa:np) {
      ...
      dat <- d2Phi(aa, bb) +
        sum(.McD[[aa]] * etaP[, bb]) +
        sum(.McD[[bb]] * etaP[, aa]) +
        as.numeric(t(etaP[, aa]) %*% H %*% etaP[, bb]) +
        sum(gPhi * e_ab)
      ld <- Cpp(aa, bb) +
        sum(.CpeRow[[aa]] * etaP[, bb]) +
        sum(.CpeRow[[bb]] * etaP[, aa]) +
...
#' @noRd
.foceiAnalyticSubjectRfoceFRCpp <- function(
  E,
  ehat,
  Om,
  neta,
...
#' @noRd
.foceiAnalyticSubjectRFRCpp <- function(
  E,
  ehat,
  Om,
  neta,
  ...
  AthC <- array(E$Ath, c(nobs, neta, ndir * ndir))
  AthRC <- array(E$AthR, c(nobs, neta, ndir * ndir))
  dOiC <- array(0, c(neta, neta, max(nom, 1L)))
  if (nom > 0L) {
    for (k in seq_len(nom)) {
      dOiC[,, k] <- omd$dOi[[k]]
...
#' sigma, then Omega.
#' @noRd
.foceiAnalyticSubjectR <- function(
  E,
  ehat,
  Om,
  ef,
  ...
  if (identical(as.integer(interaction), 0L)) {
    return(.foceiAnalyticSubjectRfoce(
      E,
      ehat,
      Om,
      ef,
  ...
  dHtD <- lapply(di, function(s) {
    ...
    for (l in ei) {
      for (m in ei) {
        D[l, m] <- sum(pf$p1 * a[, s] * a[, l] * a[, m] + pf$p * A[, l, s] * a[, m] + pf$p * a[, l] * A[, m, s])
      }
    }
    D
  })
            ...
            pf$p2 *
              ...
              pf$p1 *
                (A[, s, t] *
                  ...
                  a[, t] * A[, l, s] * a[, m] +
                  a[, t] * a[, l] * A[, m, s]) +
              pf$p * (Ath[, l, s, t] * a[, m] + A[, l, s] * A[, m, t] + A[, l, t] * A[, m, s] + a[, l] * Ath[, m, s, t])
          )
        }
      }
      D
      ...
      for (t in di) {
        Tn[l, s, t] <- sum(
          rd$r3 *
            ...
            a[, s] *
            a[, t] +
            rd$r2 * (A[, l, s] * a[, t] + A[, l, t] * a[, s] + A[, s, t] * a[, l]) +
            rd$r1 * Ath[, l, s, t]
        )
      }
    }
  ...
  d2HtEtaP <- function(p, l) {
    ...
    for (s in ei) {
      for (m in ei) {
        D[s, m] <- sum(P$pf * a[, l] * a[, s] * a[, m] + P$ps * (A[, s, l] * a[, m] + a[, s] * A[, m, l]))
      }
    }
    D
  }
  ...
  Smat <- function(p) {
    ...
    for (l in ei) {
      for (s in di) {
        M[l, s] <- sum(P$rff * a[, s] * a[, l] + P$rf * A[, l, s])
      }
    }
    M
  }
  ...
  if (!is.finite(rcond(Ht)) || rcond(Ht) < 1e-10) {
    return(NULL)
  }
  ...
  dHtStar <- lapply(1:npE, function(p) {
    D <- dHt_p(p)
    for (l in ei) {
      D <- D + dHtD[[l]] * etaP[l, p]
    }
  ...
  d2HtStar <- function(aa, bb) {
    D <- d2Ht_pp(aa, bb)
    for (l in ei) {
      D <- D + d2HtEtaP(aa, l) * etaP[l, bb] + d2HtEtaP(bb, l) * etaP[l, aa]
    }
    for (l in ei) {
      for (m in ei) {
        D <- D + d2HtDD[[l]][[m]] * etaP[l, aa] * etaP[m, bb]
      }
    }
    for (l in ei) {
      D <- D + dHtD[[l]] * .e2[[aa]][[bb]][l]
    }
    D
  }
  ...
  .nodeParts <- function(Ek, etak) {
    ...
    for (l in ei) {
      for (m in ei) {
        Hk[l, m] <- Hk[l, m] + sum(r2k * ak[, l] * ak[, m] + r1k * Ak[, l, m])
      }
    }
    Nk <- matrix(0, neta, ndir)
    for (l in ei) {
      for (d in di) {
        Nk[l, d] <- sum(r2k * ak[, l] * ak[, d] + r1k * Ak[, l, d])
      }
    }
    gk <- as.numeric(Oi %*% etak) + vapply(ei, function(l) sum(r1k * ak[, l]), numeric(1))
    PVperk <- function(p) lapply(ef$per[[sgi(p)]], evk)
    PVpairk <- function(aa, bb) {
      s1 <- sgi(aa)
      s2 <- sgi(bb)
      key <- if (paste0(s1, s2) %in% names(ef$pair)) paste0(s1, s2) else paste0(s2, s1)
    ...
    Mk <- function(p) {
      ...
      if (t == "sg") {
        return(as.numeric(crossprod(aek, PVperk(p)$rf)))
      }
    ...
    dPk <- function(p) {
      t <- typ(p)
      if (t == "th") {
        return(sum(r1k * ak[, .dirOf(p)]))
      }
      if (t == "sg") {
        return(sum(PVperk(p)$rs))
      }
    ...
    d2Pk <- function(aa, bb) {
      ...
      if (ta == "om" && tb == "om") {
        return(0.5 * as.numeric(t(etak) %*% omd$d2Oi[[omc(aa)]][[omc(bb)]] %*% etak) + 0.5 * omd$d2LD[omc(aa), omc(bb)])
      }
      if (ta == "om" || tb == "om") {
        return(0)
      }
      if (ta == "sg" && tb == "sg") {
        return(sum(PVpairk(aa, bb)$rss))
      }
      thp <- if (ta == "th") aa else bb
      sg <- if (ta == "th") bb else aa
  ...
  for (k in seq_len(nn)) {
    ...
    if (is.null(Ek)) {
      return(NULL)
    } # node solve failed -> caller drops to FD
    ...
    vK[[k]] <- matrix(
      vapply(1:npE, function(p) etaP[, p] + sqrt(2) * as.numeric(dGinvL[[p]] %*% x), numeric(neta)),
      nrow = neta
    )
  }
  lg <- lg - max(lg)
  pk <- exp(lg)
  pk <- pk / sum(pk) # pi_k (softmax, shifted for stability)
...
#' [.foceiAnalyticSubjectR].
#' @noRd
.foceiAnalyticSubjectRfoce <- function(
  E,
  ehat,
  Om,
  ef,
  ...
  fq <- if (.cf0) {
    list(
      qf0 = evf(.fc$f0$qf0),
      qff0 = evf(.fc$f0$qff0),
      qf0f0 = evf(.fc$f0$qf0f0),
      pFf0 = evf(.fc$f0$pFf0),
      pFf0f0 = evf(.fc$f0$pFf0f0),
      rhof0 = evf(.fc$f0$rhof0),
      rhof0f0 = evf(.fc$f0$rhof0f0)
    )
  } else {
    NULL
  }
  ...
  Ndat <- matrix(0, neta, ndir)
  for (l in ei) {
    for (d in di) {
      Ndat[l, d] <- sum(rd$r2 * a[, l] * a[, d] + rd$r1 * A[, l, d])
    }
  } # Phi_(eta,theta)
  ...
  for (l in ei) {
    for (m in ei) {
      Hf[l, m] <- Hf[l, m] + sum(qd$q1 * a[, l] * a[, m] + qd$q0 * A[, l, m])
    }
  } # S_eta = Hf
  HfInv <- solve(Hf)
  Nf <- matrix(0, neta, ndir)
  for (l in ei) {
    for (d in di) {
      Nf[l, d] <- sum(qd$q1 * a[, l] * a[, d] + qd$q0 * A[, l, d])
    }
  } # S_(eta,theta)
  Tnf <- array(0, c(neta, ndir, ndir))
  for (l in ei) {
      ...
      for (t in di) {
        Tnf[l, s, t] <- sum(
          qd$q2 *
            ...
            a[, s] *
            a[, t] +
            qd$q1 * (A[, l, s] * a[, t] + A[, l, t] * a[, s] + A[, s, t] * a[, l]) +
            qd$q0 * Ath[, l, s, t]
        )
      }
    }
  ...
  pairKey <- function(aa, bb) {
    s1 <- sgi(aa)
    s2 <- sgi(bb)
    if (paste0(s1, s2) %in% names(.fc$pair)) paste0(s1, s2) else paste0(s2, s1)
  }
  ...
  McolData <- function(p) {
    ...
    if (t == "th") {
      v <- Ndat[, .dirOf(p)]
      if (.cf0) {
        v <- v + as.numeric(crossprod(ae, fq$qf0 * a0[, .dirOf(p)]))
      }
      return(v)
    }
    if (t == "sg") {
      return(as.numeric(crossprod(ae, perR(p)$rf)))
    }
    as.numeric(omd$dOi[[omc(p)]] %*% ehat)
  }
  d2Phi <- function(aa, bb) {
    ta <- typ(aa)
    tb <- typ(bb) # Phi_(p,p) at fixed eta
    if (ta == "th" && tb == "th") {
      da <- .dirOf(aa)
      db <- .dirOf(bb)
      ...
      if (.cf0) {
        v <- v +
          sum(
            fq$qf0 *
              (a[, da] * a0[, db] + a0[, da] * a[, db]) +
              fq$rhof0f0 * a0[, da] * a0[, db] +
              fq$rhof0 * A0[, da, db]
          )
      }
      return(v)
    ...
    if (ta == "sg" && tb == "sg") {
      return(sum(pairR(aa, bb)$rss))
    }
    thp <- if (ta == "th") aa else bb
    sg <- if (ta == "th") bb else aa
    v <- as.numeric(crossprod(a[, .dirOf(thp)], perR(sg)$rf))
    if (.cf0) {
      v <- v + as.numeric(crossprod(a0[, .dirOf(thp)], PVf0(sg)$rhof0s))
    }
    v
  }
  ...
  McolEBE <- function(p) {
    ...
    if (t == "sg") {
      return(as.numeric(crossprod(ae, perQ(p)$qs)))
    }
    as.numeric(omd$dOi[[omc(p)]] %*% ehat)
  }
  SmatEBE <- function(p) {
    t <- typ(p) # S_(p,eta), eta-role s
    if (t == "th") {
      d <- .dirOf(p)
      M <- matrix(0, neta, ndir)
      for (l in ei) {
    ...
    for (l in ei) {
      for (s in di) {
        M[l, s] <- sum(P$qsf * a[, s] * a[, l] + P$qs * A[, l, s])
      }
    }
    M
  }
  SvecEBE <- function(aa, bb) {
    ta <- typ(aa)
    tb <- typ(bb) # S_(p,p)
    if (ta == "th" && tb == "th") {
      da <- .dirOf(aa)
      ...
      if (.cf0) {
        w <- fq$qff0 *
          (a[, da] * a0[, db] + a0[, da] * a[, db]) +
          fq$qf0f0 * a0[, da] * a0[, db] +
          fq$qf0 * A0[, da, db]
        for (l in ei) {
          v[l] <- v[l] + sum(w * a[, l] + fq$qf0 * (a0[, da] * A[, l, db] + a0[, db] * A[, l, da]))
        }
      }
      return(v)
    }
    ...
    if (ta == "sg" && tb == "sg") {
      return(as.numeric(crossprod(ae, pairQ(aa, bb)$qss)))
    }
    thp <- if (ta == "th") aa else bb
    sg <- if (ta == "th") bb else aa
    v <- SmatEBE(sg)[, .dirOf(thp)]
    if (.cf0) {
      v <- v + as.numeric(crossprod(ae, PVf0(sg)$qf0s * a0[, .dirOf(thp)]))
    }
    v
  }
  ...
  dHt_p <- function(p) {
    ...
    if (t == "th") {
      ...
      if (.cf0) {
        ...
        for (m in ei) {
          for (n in ei) {
            D[m, n] <- D[m, n] + sum(fq$pFf0 * a0[, d] * a[, m] * a[, n])
          }
        }
      }
      return(D)
  ...
  d2Ht_pp <- function(aa, bb) {
    ...
    if (.cf0) {
      ...
      for (m in ei) {
        for (n in ei) {
          D[m, n] <- D[m, n] + sum(P$pFf0s * a0[, d] * a[, m] * a[, n])
        }
      }
    }
    D
...
.foceiAnalyticSolveFA <- function(aug, params, ev, times, tol = 1e-10) {
  ...
  if (nrow(.d) == 0L || nrow(.d) != length(times) || !all(c("rx_predf_", paste0("rx_f1_", .fD)) %in% names(.d))) {
    return(NULL)
  }
  no <- nrow(.d)
  a <- matrix(0, no, nd)
  a[, match(.fD, dirs)] <- vapply(.fD, function(q) .d[[paste0("rx_f1_", q)]], numeric(no))
  A <- array(0, c(no, nd, nd))
  for (r in seq_len(nrow(aug$P2))) {
    # only i<=j emitted -> mirror to i>j (f-dir pairs)
    .ii <- match(aug$P2$i[r], dirs)
    .jj <- match(aug$P2$j[r], dirs)
    .v2 <- .d[[paste0("rx_f2_", aug$P2$i[r], "_", aug$P2$j[r])]]
    A[, .ii, .jj] <- .v2
    A[, .jj, .ii] <- .v2
  }
  .out <- list(f = .d$rx_predf_, a = a, A = A)
  if (isTRUE(aug$hasRvar)) {
    .out <- c(.out, .foceiReadRvar(.d, aug, no))
  }
  .out
}
  ...
  if (length(.sig) > 0L) {
    ...
    .out$RsigDir <- array(
      vapply(
        .sig,
        function(n) {
          vapply(dirs, function(q) .d[[paste0("rx_rsig1_", n, "_", q)]], numeric(no))
        },
        matrix(0, no, nd)
      ),
      c(no, nd, length(.sig))
    ...
    for (a in seq_along(.sig)) {
      for (b in seq_along(.sig)[seq_along(.sig) >= a]) {
        .v <- .d[[paste0("rx_rsig2_", .sig[a], "_", .sig[b])]]
        .Rs2[, a, b] <- .v
        .Rs2[, b, a] <- .v
      }
    }
...
.foceiCovAnalyticCalc <- function(fit) {
  ...
  if (
    isTRUE(as.logical(rxode2::rxGetControl(ui, "fo", FALSE))) ||
  ...
  .idf0 <- ui$iniDf
  if (any(!is.na(.idf0$condition) & .idf0$condition != "id" & is.na(.idf0$err))) {
    # IOV
  ...
  if (is.null(ef)) {
    return(NULL)
  } # unsupported error model -> errFull already messaged
  ...
  if (any(.iniIsFixed(ini, thetaForEta))) {
    # fixed structural theta breaks eta indexing
    return(.foceiAnalyticFallback("a fixed mu-referenced structural parameter"))
  }
  keep <- !.iniIsFixed(ini, ef$sgName)
  ef$sgVar <- ef$sgVar[keep]
