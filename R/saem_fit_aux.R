...
.saemLlObsMask <- function(saemCfg, ixEndpnt) {
  ...
  .bad <- function() {
    stop("saem.cfg has a general likelihood but no 'res.mod'; cannot tell ll() observations apart", call. = FALSE)
  }
  if (!is.null(.resMod)) {
    ...
    if (.gen && !any(.resMod == 0L)) {
      .bad()
    }
    return(.resMod[ixEndpnt] == 0L)
  }
  if (!.gen) {
    return(rep(FALSE, length(ixEndpnt)))
  }
  if (max(ixEndpnt) > 1L) {
    .bad()
  }
...
#' @noRd
.saemGqNodes <- function(nnodes, nphi1, maxNodes = getOption("nlmixr2.saemGqMaxNodes", 50000)) {
  nnodes <- min(nnodes, 25)
  if (nnodes <= 1 || nphi1 <= 0) {
    return(nnodes)
  }
  if (nnodes^nphi1 <= maxNodes) {
    return(nnodes)
  }
  ...
  if (nphi > 1L) {
    for (.j in seq_len(nphi - 1L)) {
      for (.k in seq(.j + 1L, nphi)) {
        .v <- (.ev(.j, 1, .k, 1) - .ev(.j, 1, .k, -1) - .ev(.j, -1, .k, 1) + .ev(.j, -1, .k, -1)) /
          (4 * .h[id, .j] * .h[id, .k])
        .ret[, .j, .k] <- .v
        .ret[, .k, .j] <- .v
      }
...
#' @noRd
.saemLlSubjectPart <- function(i, ix, DF, g, isLL, d2LL, omega, iOmega, nphi, i1, mcov, covEstIx, doVar, nom, omPairs) {
  ...
  .M <- .H -
    .H[, i1, drop = FALSE] %*%
      solve(iOmega + .H[i1, i1, drop = FALSE]) %*%
      .H[i1, , drop = FALSE]
  .M <- 0.5 * (.M + t(.M))
...
calc.COV <- function(fit0) {
  ...
  if (.covFull) {
    ...
    if (nrow(.ri) > 0L) {
      ...
      if (anyNA(.resEndpnt)) {
        ...
        warning("saem covFull: residual parameter endpoint unknown; variance block dropped", call. = FALSE)
        .resNames <- character(0)
        .resType <- integer(0)
        .resEndpnt <- integer(0)
        .omPairs <- NULL
  ...
  .parts <- lapply(1:N, function(i) {
    ...
    if (inherits(VD, "try-error")) {
      stop("Spectral decom failure when computing FIM")
    }
    D <- Re(VD$values)
    V <- Re(VD$vectors)
    m <- diag(length(D))
