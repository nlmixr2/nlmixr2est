...
.configsaem <- function(model, data, inits,
  ...
  cfg <- list(
    ...
    burnAuto = as.integer(burnAuto),
    burnWindow = as.integer(burnWindow),
    burnAlpha = as.numeric(burnAlpha),
    nb_fixResid=nb_fixResid,
    niter_phi0 = niter_phi0,
    nmc = nmc,
    ## saemControl() arguments now, not constants.  rmcmc/coef_sa/coef_phi0
    ## were saemix's rw.init / alpha.sa / its phi0 analogue, ported as their
    ## STARTING values with the adaptation left behind (see `iaccept`).
    coef_phi0 = coefPhi0,
    rmcmc = rmcmc,
    coef_sa = coefSa,
    iaccept = iaccept,
    ...
    etaDistSpreadGuard = as.integer(etaDistSpreadGuard),
