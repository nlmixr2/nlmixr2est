...
# foceiLinCmtCarryEligible.R, gated in foceiLinCmtCarryGates.R) the ordinary
# symengine expansion of d(rx_pred_)/d(eta) is WRONG whenever a covariate
# makes that parameter vary within a subject: linCmtB()'s production
# Jacobian reconstructs the carried Alast sensitivity assuming theta is
# constant across the subject.  This file replaces the naive generated line
# for such a pair with lines that drive the exact carry recurrence through
# linCmtB()'s which1=-5/-6/-7 sentinels (rxode2's per-subject
# ind->linCmtCarryT storage):
#
