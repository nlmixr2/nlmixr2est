...
#'   (which only estimates parameters that occupy the latent space).  A small eta
#'   is injected so the parameter enters the latent space, and the reported fixed
#'   effect is `theta + mean(eta)` with the temporary eta dropped from the output
#'   model.
#'
