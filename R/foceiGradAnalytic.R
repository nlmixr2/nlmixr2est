...
#' @noRd
.foceiAnalyticSubjectGrad <- function(
  E,
  ehat,
  Om,
  ef,
  ...
  dPhi_p <- function(p) {
    t <- typ(p)
    if (t == "th") {
      return(sum(rd$r1 * a[, dirTh[p]]))
    }
    if (t == "sg") {
      return(sum(PVper(p)$rs))
    }
  ...
  for (p in seq_len(np)) {
    ...
    dHtStar <- dHt_p(p)
    for (l in ei) {
      dHtStar <- dHtStar + etaP[l, p] * dHtD[[l]]
    }
...
#' @noRd
.foceiAnalyticSubjectGradFR <- function(
  E,
  ehat,
  Om,
  neta,
  ...
  res <- y - f
  rf <- -res / R
  rR <- 0.5 * (1 / R - res^2 / R^2) # rho_f, rho_R
  rff <- 1 / R
  rfR <- res / R^2
  rRR <- 0.5 * (-1 / R^2 + 2 * res^2 / R^3) # rho_ff/_fR/_RR
  eff <- 1 / R
  eRR <- 0.5 / R^2 # E[rho_ff], E[rho_RR] at y=f
  nom <- length(dOiEst)
  np <- nth + nsg + nom
  ei <- seq_len(neta)
  di <- seq_len(ndir)
  ...
  for (l in ei) {
    for (m in ei) {
      Ht[l, m] <- Ht[l, m] + sum(eff * a[, l] * a[, m] + eRR * aR[, l] * aR[, m])
    }
  }
  Hti <- solve(Ht)
      ...
      for (m in ei) {
        D[l, m] <- sum(
          -aR[, s] /
            ...
            a[, l] *
            a[, m] +
            eff * (A[, l, s] * a[, m] + a[, l] * A[, m, s]) +
            -aR[, s] / R^3 * aR[, l] * aR[, m] +
            eRR * (AR[, l, s] * aR[, m] + aR[, l] * AR[, m, s])
        )
      }
    }
    D
        ...
        for (m in ei) {
          D[l, m] <- sum(
            -Rsig[, .c] /
              ...
              a[, l] *
              a[, m] -
              Rsig[, .c] / R^3 * aR[, l] * aR[, m] +
              eRR * (RsigDir[, l, .c] * aR[, m] + aR[, l] * RsigDir[, m, .c])
          )
        }
      }
      D
  ...
  dPhi_p <- function(p) {
    t <- typ(p)
    if (t == "th") {
      return(sum(rf * a[, dirTh[p]] + rR * aR[, dirTh[p]]))
    }
    if (t == "sg") {
      return(sum(rR * Rsig[, sigCol[p - nth]]))
    }
    0.5 * as.numeric(t(ehat) %*% dOiEst[[omc(p)]] %*% ehat) - tr28[omc(p)]
  }
  etaP <- vapply(seq_len(np), function(p) as.numeric(-HiM %*% Mcol(p)), numeric(neta))
...
.foceiAnalyticSubjectGradAgqFR <- function(
  E,
  Eks,
  ehat,
  Om,
  ...
  if (is.null(H0) || !is.finite(rcond(Ht)) || rcond(Ht) < 1e-10) {
    return(NULL)
  } # Ht = H0'H0, H0 upper triangular
  Ginv <- backsolve(H0, diag(neta)) # H0^-1
  dHtStarL <- vector("list", np)
  dGinvL <- vector("list", np)
  for (p in seq_len(np)) {
    .dS <- dHt_p(p)
    for (l in ei) {
      .dS <- .dS + etaP[l, p] * dHtD[[l]]
    }
    dHtStarL[[p]] <- .dS
    dGinvL[[p]] <- -Ginv %*% .phiU(t(Ginv) %*% .dS %*% Ginv)
  }
  # log joint density up to an eta-independent constant (cancels in pi_k)
  .lf <- function(Ee, eta) -0.5 * sum(log(Ee$R) + (Ee$y - Ee$f)^2 / Ee$R) - 0.5 * as.numeric(t(eta) %*% Oi %*% eta)
  l0 <- .lf(E, ehat)
  nn <- nrow(qx)
  lognode <- numeric(nn)
  perNode <- matrix(0, nn, np)
  for (k in seq_len(nn)) {
    x <- qx[k, ]
    w <- qw[k, ]
    Ek <- Eks[[k]]
    ...
    .Rk <- Ek$R
    rfk <- -.resk / .Rk
    rRk <- 0.5 * (1 / .Rk - .resk^2 / .Rk^2)
    # Phi_eta at the NODE: nonzero (the envelope theorem does not apply off the mode)
    gPhik <- as.numeric(Oi %*% etaCur)
    for (l in ei) {
      gPhik[l] <- gPhik[l] + sum(rfk * Ek$a[, l] + rRk * Ek$aR[, l])
    }
    for (p in seq_len(np)) {
      .t <- typ(p)
      .dphi <- if (.t == "th") {
        sum(rfk * Ek$a[, dirTh[p]] + rRk * Ek$aR[, dirTh[p]])
      } else if (.t == "sg") {
        sum(rRk * Ek$Rsig[, sigCol[p - nth]])
      } else {
        # the -tr28 constant is node-independent; sum_k pi_k = 1 passes it through
        0.5 * as.numeric(t(etaCur) %*% dOiEst[[omc(p)]] %*% etaCur) - tr28[omc(p)]
      }
      perNode[k, p] <- .dphi + sum(gPhik * (etaP[, p] + sqrt(2) * as.numeric(dGinvL[[p]] %*% x)))
    }
  }
  if (!all(is.finite(lognode))) {
    return(NULL)
  }
  pk <- exp(lognode - max(lognode))
  pk <- pk / sum(pk) # quadrature posterior weights
  g <- vapply(seq_len(np), function(p) 2 * sum(pk * perNode[, p]) + tr(Hti %*% dHtStarL[[p]]), numeric(1))
  list(g = g, etaP = etaP)
}
...
#' @noRd
.foceiAnalyticSubjectGradFoceFR <- function(
  E,
  ehat,
  Om,
  neta,
  ...
  if (.fp) {
    ...
    R0 <- E0$R
    aRe <- matrix(0, nrow(a), ndir)
    aRc <- E0$aR
    R0sig <- E0$Rsig
  }
  res <- y - f
  rho_f <- -res / R0
  rho_R <- 0.5 * (1 / R0 - res^2 / R0^2)
  q0 <- -res / R0
  q1 <- 1 / R0 # interaction-free inner gradient/curv
  nom <- length(dOiEst)
  np <- nth + nsg + nom
  ae <- a[, ei, drop = FALSE]
  ...
  for (l in ei) {
    gPhi[l] <- gPhi[l] + sum(rho_f * a[, l] + rho_R * aRe[, l])
  }
  ...
  for (l in ei) {
    for (m in ei) {
      Hf[l, m] <- Hf[l, m] + sum(q1 * a[, l] * a[, m] + q0 * A[, l, m] + res / R0^2 * a[, l] * aRe[, m])
  ...
  dHtD <- lapply(di, function(s) {
    ...
    for (l in ei) {
      for (m in ei) {
        D[l, m] <- sum(-aRe[, s] / R0^2 * a[, l] * a[, m] + (A[, l, s] * a[, m] + a[, l] * A[, m, s]) / R0)
      }
    }
    D
  })
  ...
  McolEBE <- function(p) {
    ...
    if (t == "th") {
      d <- dirTh[p]
      return(Nf[, d] + as.numeric(crossprod(ae, (res / R0^2) * aRc[, d])))
    }
    if (t == "sg") {
      return(as.numeric(crossprod(ae, (res / R0^2) * R0sig[, sigCol[p - nth]])))
    }
    as.numeric(dOiEst[[omc(p)]] %*% ehat)
  }
  ...
  dHt_p <- function(p) {
    ...
    if (t == "sg") {
      return(ouRc(-R0sig[, sigCol[p - nth]] / R0^2))
    }
    dOiEst[[omc(p)]]
  }
  # explicit dPhi/dp (fixed eta): rho_f df/dp + rho_R dR0/dp (columns use aRc)
  dPhiExplicit <- function(p) {
    t <- typ(p)
    if (t == "th") {
      d <- dirTh[p]
      return(sum(rho_f * a[, d] + rho_R * aRc[, d]))
    }
    if (t == "sg") {
      return(sum(rho_R * R0sig[, sigCol[p - nth]]))
    }
    0.5 * as.numeric(t(ehat) %*% dOiEst[[omc(p)]] %*% ehat) - tr28[omc(p)]
  }
...
#' @noRd
.foceiAnalyticSubjectGradFRCpp <- function(
  E,
  ehat,
  Om,
  neta,
...
#' @noRd
.foceiAnalyticSubjectGradFoceFRCpp <- function(
  E,
  ehat,
  Om,
  neta,
...
#' @noRd
.foceiAnalyticSubjectGradFoce <- function(
  E,
  ehat,
  Om,
  ef,
  ...
  McolEBE <- function(p) {
    ...
    if (t == "sg") {
      return(as.numeric(crossprod(ae, evf(.fc$per[[sgi(p)]]$qs))))
    }
    as.numeric(dOiEst[[omc(p)]] %*% ehat)
  }
  ...
  dHt_p <- function(p) {
    ...
    if (t == "sg") {
      return(ouAA(evf(.fc$per[[sgi(p)]]$psF)))
    }
    dOiEst[[omc(p)]]
  }
  ...
  dPhiExplicit <- function(p) {
    t <- typ(p)
    if (t == "th") {
      v <- sum(rd$r1 * a[, .dirOf(p)])
      if (.cf0) {
        v <- v + sum(fq$rhof0 * a0[, .dirOf(p)])
      }
      return(v)
    }
    if (t == "sg") {
      return(sum(evf(.fc$per[[sgi(p)]]$rs)))
    }
    0.5 * as.numeric(t(ehat) %*% dOiEst[[omc(p)]] %*% ehat) - tr28[omc(p)]
  }
  etaP <- vapply(seq_len(np), function(p) as.numeric(-HfInv %*% McolEBE(p)), numeric(neta))
