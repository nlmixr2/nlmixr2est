...
.genAgq <- function(n = 64) {
  ...
  .f <- function(n) {}
  body(.f) <- as.call(c(list(quote(`{`)), str2lang(paste0("if (is.na(n)) return(", n, ")")), .c))
  .f
}
...
.nlmixr2estbuild <- function() {
  ...
  writeLines(c("## created by .nlmixr2estbuild() in build.R edit there", .tmp), devtools::package_file("R/agqGen.R"))
  message("done")
