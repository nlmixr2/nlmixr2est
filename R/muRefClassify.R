...
#' A user-fixed (`fix()`, `literalFix=FALSE`) population theta likewise
#' excludes the group (the regression would move the fixed value). A bound
#' on one covariate-coefficient theta excludes only that covariate (marked
#' `bounded=TRUE`, treated like a time-varying covariate), while the
#' population theta and other unbounded covariates in the group still get
#' the mu-ref speed-up. Only affects this family (`mufocei`/`irlsfocei`/
...
#' groups (empty `covariates`); fixed or bounded plain thetas are excluded
#' quietly (they stay ordinary outer-optimized parameters). If profiling
#' the plain groups would leave the outer optimizer with no free parameter
#' at all, the plain groups are dropped (covariate groups kept).
#'
#' @param ui rxode2 ui object (post mu2-hook, if applicable)
...
#'   covariateParameter=, bounded=))`, one element per group whose
#'   population theta is unbounded and not user-fixed
#' @author Matthew L. Fidler
#' @noRd
