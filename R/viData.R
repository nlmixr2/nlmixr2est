...
.adviClassifyPars <- function(ui) {
  ...
  list(
    ...
    muRefThetaIdx = .muRefThetaIdx,
    struct = .sens$struct,
    sigma = .sens$sigma,
    thetaSensIdx = .sens$all
  )
}
...
.adviDataPrep <- function(ui, data) {
  ...
  if (.neta == 0L) {
    stop("variational inference requires at least one random effect", call. = FALSE)
  }
  .idf <- ui$iniDf
  ...
  .thetaFix <- as.logical(.thRows$fix)
  .thetaFix[is.na(.thetaFix)] <- FALSE
  ...
  if (is.null(d$EVID)) {
    d$EVID <- ifelse(is.na(d$AMT) | d$AMT == 0, 0L, 1L)
  }
  .ids <- unique(d$ID)
  N <- length(.ids)
  subj <- vector("list", N)
  ...
  for (i in seq_len(N)) {
    ...
    subj[[i]] <- list(ev = .di, times = .obs$TIME, y = .obs$DV, n = nrow(.obs), cens = .cens, limit = .limit)
    .nobs <- .nobs + nrow(.obs)
  }
  ...
  list(
    ...
    muRefThetaIdx = .cls$muRefThetaIdx,
    isFree = is.na(.cls$muRefThetaIdx),
    structIdx = .cls$struct,
    sigmaIdx = .cls$sigma,
    thetaSensIdx = .cls$thetaSensIdx,
    omega = .omega,
    omegaFix = .omegaFix,
