...
public:
  ...
  void inits(List x) {
    ...
    // PR #665 mixture support adds (nMix - 1) printed mixture-probability
    // parameters to the parameter-history vector.  The per-parameter
    // transform wiring (xPar / logit / probit bounds) that PR #665
    // originally set by hand here is now handled uniformly by
    // scaleAttachXform(x["xform"]) below (the post-#654 shared scale.h
    // path); parHistNames already includes ui$mixProbs, so the xform
    // sub-list covers the mixture-probability entries too.
