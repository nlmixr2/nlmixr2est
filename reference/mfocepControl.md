...
- interaction:
  ...
  it cannot be changed, use \`mfocei\` instead

- muModel:

  Selects the regression variant; for \`mfocepControl()\` this is always
  \`"lin"\` and cannot be changed – use \`ifocepControl()\` for the IRLS
  variant.

- foce:

  FOCE residual-variance mode; for \`mfocepControl()\` this is always
  \`"foce+"\` and cannot be changed – use \`mfoceControl()\` for
  \`"nonmem"\`
