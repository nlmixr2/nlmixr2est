...
``` r
mfoceiControl(sigdig = 4, ..., muModel = c("lin", "irls", "none"))
```
...
- muModel:
  ...
  Selects the regression variant; for \`mfoceiControl()\` this is always
  \`"lin"\` and cannot be changed – use \`ifoceiControl()\` for the IRLS
  variant.
