...
``` r
ifoceiControl(sigdig = 4, ..., muModel = c("irls", "lin", "none"))
```
...
- muModel:
  ...
  Selects the regression variant; for \`ifoceiControl()\` this is always
  \`"irls"\` and cannot be changed – use \`mfoceiControl()\` for the
