...
``` r
mulaplaceControl(sigdig = 3, ..., nAGQ = 1, muModel = c("lin", "irls", "none"))
```
...
- muModel:
  ...
  Selects the regression variant; for \`mulaplaceControl()\` this is
  always \`"lin"\` and cannot be changed – use \`irlslaplaceControl()\`
