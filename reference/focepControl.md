...
\`foce = "foce+"\` option of \[foceiControl()\]). It is the \`foce\`
method with \`foce = "foce+"\` forced; use \`foce\` (est = "foce") for
the NONMEM-matching frozen-R behavior.
...
- foce:
  ...
  FOCE residual-variance mode; for \`focepControl()\` this is always
  \`"foce+"\` and cannot be changed – use \`foceControl()\` for
  \`"nonmem"\`
