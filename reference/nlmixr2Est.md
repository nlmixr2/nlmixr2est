...
cannot use priors at all - \`"normal"\` – normal priors only
(\`dnorm()\`, \`stdNormal()\` and the \`multiNormal()\` family) -
\`"nwpri"\` – normal priors and degrees of freedom on an omega block
(\`invWishart(4)\`), as a NONMEM NWPRI model works; a normal prior on
the omega values themselves is refused - \`"all"\` – the method handles
