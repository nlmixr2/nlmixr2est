...
- perNoCor:
  ...
  The resolved absolute iteration is stored with the fit and reused by
  \`resume=\`, so a resumed run releases the correlations at the same
  global iteration a single long run would – recomputing the fraction
  from the resumed call's \`iters\` would re-apply a hold the original
  run had passed.
