...
A convenience wrapper around \[impmapControl()\] with \`mapIter = 0\`,
i.e. the importance-sampling proposal is centered at the running
conditional mean rather than re-optimized to the MAP mode each iteration
(NONMEM METHOD=IMP). See \[impmapControl()\] for the full parameter
list.
