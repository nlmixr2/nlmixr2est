...
``` r
mlaplaceControl(sigdig = 4, ..., nAGQ = 1, muModel = c("lin", "irls", "none"))
```
...
- muModel:
  ...
  Selects the regression variant; for \`mlaplaceControl()\` this is
  always \`"lin"\` and cannot be changed – use \`ilaplaceControl()\` for
  the IRLS variant.
