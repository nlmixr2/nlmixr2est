...
``` r
irlsfoceiControl(sigdig = 3, ..., muModel = c("irls", "lin", "none"))
```
...
- muModel:
  ...
  Selects the regression variant; for \`irlsfoceiControl()\` this is
  always \`"irls"\` and cannot be changed – use \`mufoceiControl()\` for
