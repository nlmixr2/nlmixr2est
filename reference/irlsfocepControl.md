...
\`irlsfoce\` with \`foce = "foce+"\` forced (see
\`foceiControl(foce=)\`).
...
- interaction:
  ...
  it cannot be changed, use \`irlsfocei\` instead

- muModel:

  Selects the regression variant; for \`irlsfocepControl()\` this is
  always \`"irls"\` and cannot be changed – use \`mufocepControl()\` for
  the closed-form OLS variant.

- foce:

  FOCE residual-variance mode; for \`irlsfocepControl()\` this is always
  \`"foce+"\` and cannot be changed – use \`irlsfoceControl()\` for
  \`"nonmem"\`

## Value

irlsfocepControl object
